// asml_signal_bot.cpp — ASML buy/sell signal engine, pure C++
// ============================================================
...
//
// build: clang++ -std=c++17 -O2 -o asml_signal_bot asml_signal_bot.cpp
// run:   ./asml_signal_bot            (spawns bridge)
//        ./asml_signal_bot --stdin    (replay: feed bar lines for testing)
...
// ---- params: por-ticker via env ASML_* (orden Yunior 2026-07-10 "tune per
...
static const int    BB_N = 20;      static const double BB_STD = envd("ASML_BB_STD", 3.0);
static const int    RSI_N = 14;     static const double RSI_OS = envd("ASML_RSI_OS", 25.0);
static const int    VOL_N = 20;     static const double VOL_MULT = envd("ASML_VOL_MULT", 1.2);
static const int    ATR_N = 14;
static const int    CONFIRM_WINDOW = 60;
static const double TARGET_PCT = envd("ASML_TARGET", 4.0),
                    FLOOR_PCT  = envd("ASML_FLOOR", 1.0),
                    TRAIL_ATR  = envd("ASML_TRAIL_ATR", 3.0);
// HARD STOP alert (orden 2026-07-10): a -ASML_STOP% del entry el bot GRITA la
// perdida (SELL-STOP) en vez de callarse a esperar el floor. Humano decide.
static const double STOP_PCT = envd("ASML_STOP", 3.0);
...
static const double SKIP_OPEN      = envd("ASML_SKIP_OPEN", 0);
static const double TIME_STOP_MIN  = envd("ASML_TIME_STOP_MIN", 0);
static const double EOD_FORCE      = envd("ASML_EOD_FORCE", 0);
static const double CONFIRM_STRICT = envd("ASML_CONFIRM_STRICT", 0);
static const double MAX_DAY        = envd("ASML_MAX_DAY", 0);
...
static const double SCORE_MIN  = envd("ASML_SCORE_MIN", 0);
static const double WHALE_USD  = envd("ASML_WHALE_USD", 75000);
...
static const double SPREAD_MAX = envd("ASML_SPREAD_MAX", 0);
// TREND MODE generico (ASML_MODE=trend): flip Supertrend 5m / ruptura max del
// dia con CUSUM >= TREND_CUSUM; TREND_VWAP=1 exige ademas c>VWAP y vol>=volMA.
static const bool TREND_MODE = [] {
    const char* v = std::getenv("ASML_MODE");
    return v && !std::strcmp(v, "trend");
}();
static const double TREND_CUSUM = envd("ASML_TREND_CUSUM", 0.01);
static const double TREND_VWAP  = envd("ASML_TREND_VWAP", 0);
// ===== v4 AMBAS DIRECCIONES (orden Yunior 2026-07-11: señales cuando sube
// Y cuando baja). ASML_SHORTS=1 activa el espejo corto: blow-off arriba de la
...
static const double SHORTS = envd("ASML_SHORTS", 0);
// exits propios del corto (los mercados caen distinto a como suben);
// sin definir heredan los del largo
static const double S_TARGET = envd("ASML_S_TARGET", TARGET_PCT);
static const double S_STOP   = envd("ASML_S_STOP", STOP_PCT);
static const double S_TRAIL  = envd("ASML_S_TRAIL", TRAIL_ATR);
static const double S_FLOOR  = envd("ASML_S_FLOOR", FLOOR_PCT);
static const double S_TSTOP  = envd("ASML_S_TSTOP", TIME_STOP_MIN);
...
static const double S_BB_STD    = envd("ASML_S_BB_STD", BB_STD);
static const double S_RSI_OS    = envd("ASML_S_RSI_OS", RSI_OS);
static const double S_VOL_MULT  = envd("ASML_S_VOL_MULT", VOL_MULT);
static const double S_SCORE_MIN = envd("ASML_S_SCORE_MIN", SCORE_MIN);
static const double S_TCUSUM    = envd("ASML_S_TREND_CUSUM", TREND_CUSUM);
...
static const double CANDLE   = envd("ASML_CANDLE", 0);
static const double S_CANDLE = envd("ASML_S_CANDLE", CANDLE);
...
static const double QUAKE_MIN    = envd("ASML_QUAKE_MIN", 0.02);
static const double QUAKE_BANNER = envd("ASML_QUAKE_BANNER", 0);
static const bool S_MODE_TREND = [] {
    const char* v = std::getenv("ASML_S_MODE");
    if (v && !std::strcmp(v, "trend")) return true;
    if (v && !std::strcmp(v, "mr")) return false;
    const char* m = std::getenv("ASML_MODE");           // hereda el modo largo
    return m && !std::strcmp(m, "trend");
}();
static const char* SPOS_FILE = "data/pos_asml_s.txt";
...
static const char* WHALE_FILE = "data/whale_asml.txt";
static const char* NBBO_FILE  = "data/nbbo_asml.txt";
...
static const char* POS_FILE = "data/pos_asml.txt";
...
static void notify(const char* title, const char* msg, bool urgent) {
    ...
    FILE* f = std::fopen("logs/asml_operations.log", "a");
    if (f) {
...
// unica con prob% (logistica calibrada por backtest; ASML_V5_A/B).
// ASML_V5=1 activa; default 0 = byte-identico al motor previo.
static const double V5        = envd("ASML_V5", 0);
static const double V5_MIN    = envd("ASML_V5_MIN", 99.0);   // score minimo (de 8)
static const double V5_A      = envd("ASML_V5_A", -3.2);    // logistica: prob = 1/(1+e^-(A+B*score))
static const double V5_B      = envd("ASML_V5_B", 0.62);
static const double V5_COOL   = envd("ASML_V5_COOL", 1800); // s entre señales mismo lado
...
static int v5_opt(char* out, size_t n) {
    FILE* f = fopen("data/options_asml.txt", "r");
...
static void v5_on_bar(const Bar& b, bool alert_hours, int H, int M) {
    ...
    if (sbuy >= V5_MIN && b.t - v5_last_buy >= V5_COOL && sbuy > ssell) {
        ...
        char m[420];
        std::snprintf(m, sizeof(m), "ASML @ %.2f | prob %.0f%% | %s | %s%s",
                      b.c, p, rb, sc, opt);
        std::printf("[%02d:%02d] *** ASML V5 BUY *** score %.1f prob %.0f%% %s t=%.0f\n",
                    H, M, sbuy, p, rb, b.t);
        std::fflush(stdout);
        notify("ASML: BUY", m, true);
        if (audio_gate(true)) { play("sounds/dram_buy.wav", "Glass"); speak("buy A S M L now"); }
    }
    if (ssell >= V5_MIN && b.t - v5_last_sell >= V5_COOL && ssell > sbuy) {
        ...
        std::snprintf(m, sizeof(m), "ASML @ %.2f | prob %.0f%% | %s | %s%s",
                      b.c, p, rs, sc, opt);
        std::printf("[%02d:%02d] *** ASML V5 SELL *** score %.1f prob %.0f%% %s t=%.0f\n",
                    H, M, ssell, p, rs, b.t);
        std::fflush(stdout);
        notify("ASML: SELL", m, true);
        if (audio_gate(true)) { play("sounds/dram_sell.wav", "Basso"); speak("sell A S M L now"); }
    }
}
// ===== FIN MOTOR v5 =====
...
// clases de señal cerradas con prob% calibrada (data/prob_table_asml.txt,
...
// que atraviesa el nivel = TRAMPA-EVITADA (cancelada + log). ASML_V6_RETEST=0
// restaura el disparo inmediato v6.0.
#include <sys/stat.h>
#include <algorithm>
#include <filesystem>

static const double V6_ON       = envd("ASML_V6", 1);           // 1 = activo
static const double V6_MIN      = envd("ASML_V6_MIN", 6.0);     // score minimo (de 10)
static const double V6_COOL     = envd("ASML_V6_COOL", 1800);   // s entre señales mismo lado
static const double V6_PROB_MIN = envd("ASML_V6_PROB_MIN", 55.0);
static const double V6_MAX_CLASS_DAY = envd("ASML_V6_MAX_CLASS_DAY", 2);
static const double V6_RVOL     = envd("ASML_V6_RVOL", 1.5);
static const double V6_RVOL_RECLAIM = envd("ASML_V6_RVOL_RECLAIM", 2.0);
static const double V6_RETR_CONT = envd("ASML_V6_RETR_CONT", 0.50);
static const double V6_RETR_REV  = envd("ASML_V6_RETR_REV", 0.62);
static const double V6_SQUEEZE_PCT = envd("ASML_V6_SQUEEZE_PCT", 10.0);
static const double V6_ADX_TREND = envd("ASML_V6_ADX_TREND", 25.0);
static const double V6_ADX_RANGE = envd("ASML_V6_ADX_RANGE", 20.0);
static const double V6_OR_LATERAL = envd("ASML_V6_OR_LATERAL", 1.2);
static const double V6_GAP_BREAKAWAY = envd("ASML_V6_GAP_BREAKAWAY", 1.0);
static const double V6_KRX      = envd("ASML_KRX", 0);          // 1 = sesion KST (open min 540)
static const double V6_DEBUG    = envd("ASML_V6_DEBUG", 0);
static const double V6_RETEST   = envd("ASML_V6_RETEST", 1);    // v6.1: 1 = retest-confirm en rupturas
static const double V6_RETEST_MAX = envd("ASML_V6_RETEST_MAX", 20); // bars 1m armada antes de expirar
...
// Override CSV: export ASML_V6_RETEST_EXEMPT="VWAP_LOSS_SHORT,ORB_SHORT"
...
static const bool v6_retest_exempt_init = []() {
    const char* v = std::getenv("ASML_V6_RETEST_EXEMPT");
...
static void v6_emit(bool isbuy, int cls, double prob, double score, const char* r,
    ...
    v6_fires_today[cls]++;
    std::printf("[%02d:%02d] *** ASML V6 %s *** prob %.0f%% clase %s score %.1f %s t=%.0f\n",
                H, M, isbuy ? "BUY" : "SELL", prob, V6_CLS[cls], score, r, b.t);
    std::fflush(stdout);
    char m[420];
    std::snprintf(m, sizeof(m), "ASML @ %.2f | prob %.0f%% | %s | fase=%s",
                  b.c, prob, r, V6_PHASE_NAME[ph]);
    notify(isbuy ? "ASML: BUY" : "ASML: SELL", m, true);
    ...
    if (audio_gate(true)) {
        if (isbuy) { play("sounds/dram_buy.wav", "Glass");
            char sp[120]; std::snprintf(sp, sizeof(sp),
                "buy A S M L now, probability %.0f percent", prob); speak(sp); }
        else { play("sounds/dram_sell.wav", "Basso");
            char sp[120]; std::snprintf(sp, sizeof(sp),
                "sell A S M L now, probability %.0f percent", prob); speak(sp); } } }
...
static void v6_on_bar(const Bar& b, bool alert_hours, int H, int M) {
    ...
    v6_prob.maybe_reload("data/prob_table_asml.txt", b.t);
    ...
    auto v6_retest_step = [&](V6Armed& a, double& blk, bool veto_now) {
        ...
        if (cross > 0 && cross > 0.5 * imp) {
            std::printf("[%02d:%02d] ASML V6 TRAMPA-EVITADA clase %s nivel %.2f px %.2f "
                        "impulso %.2f t=%.0f\n",
...
int main(int argc, char** argv) {
    ...
    FILE* in = stdin;
    if (!use_stdin) {
        // daemon IBKR escribe data/bars_asml_ibkr.txt; tail -F lo sigue 
        // (Yunior 2026-07-20: only ibkr — fuente unica)
        in = popen("tail -n +1 -F data/bars_asml_ibkr.txt 2>>logs/bridge_asml.log", "r");
        if (!in) { std::fprintf(stderr, "no bridge\n"); return 1; }
        std::fprintf(stderr, "asml_signal_bot (C++): bridge ASML 1m real iniciado\n");
    }
    ...
    while (std::fgets(line, sizeof(line), in)) {
        ...
        if (nbars > 1 && prev_close > 0) { /* prev_close ya actualizado: usar retorno del bar */ }
        {
            ...
            if (last_c_for_ret > 0) {
                ...
                if (alert_hours && vol_ok_radar && b.t - last_cusum > 3600) {
                    if (cusum_up > hthr) {
                        std::printf("[%02d:%02d] CUSUM: ASML SUBIENDO fuerte (+%.2f%% acumulado) px %.2f t=%.0f\n",
                                    H, M, cusum_up * 100, b.c, b.t);
                        std::fflush(stdout);
                        { char m[160]; std::snprintf(m, sizeof(m), "CUSUM: subiendo fuerte %+.2f%% px %.2f", cusum_up*100, b.c); notify("ASML TERREMOTO ALZA", m, QUAKE_BANNER > 0); }
                        cusum_up = 0; cusum_dn = 0; last_cusum = b.t;
                    } else if (cusum_dn < -hthr) {
                        std::printf("[%02d:%02d] CUSUM: ASML CAYENDO fuerte (%.2f%% acumulado) px %.2f t=%.0f\n",
                                    H, M, cusum_dn * 100, b.c, b.t);
                        std::fflush(stdout);
                        { char m[160]; std::snprintf(m, sizeof(m), "CUSUM: cayendo fuerte %.2f%% px %.2f", cusum_dn*100, b.c); notify("ASML TERREMOTO CAIDA", m, QUAKE_BANNER > 0); }
                        cusum_up = 0; cusum_dn = 0; last_cusum = b.t;
                    }
                }
            }
        ...
        if (bar5 && atr5 > 0 && n5 > 10) {
            ...
            if (st_trend != 0 && nt != st_trend && alert_hours && b.t - last_st > 3600) {
                if (nt > 0) {
                    std::printf("[%02d:%02d] SUPERTREND: tendencia ASML cambio a ALCISTA px %.2f\n", H, M, b.c);
                    { char m[120]; std::snprintf(m, sizeof(m), "Supertrend: tendencia ALCISTA px %.2f", b.c); notify("ASML tendencia", m, false); }
                } else {
                    std::printf("[%02d:%02d] SUPERTREND: tendencia ASML cambio a BAJISTA px %.2f\n", H, M, b.c);
                    { char m[120]; std::snprintf(m, sizeof(m), "Supertrend: tendencia BAJISTA px %.2f", b.c); notify("ASML tendencia", m, false); }
                }
                std::fflush(stdout);
                last_st = b.t;
        ...
        if ((int)dh20.size() == 390 && alert_hours && b.t - last_don > 3600) {
            ...
            if (b.c > hi) {
                std::printf("[%02d:%02d] DONCHIAN: ASML rompe maximo del dia px %.2f > %.2f\n", H, M, b.c, hi);
                std::fflush(stdout);
                { char m[120]; std::snprintf(m, sizeof(m), "Donchian: rompe maximo del dia px %.2f", b.c); notify("ASML breakout", m, false); }
                last_don = b.t;
            } else if (b.c < lo) {
                std::printf("[%02d:%02d] DONCHIAN: ASML rompe minimo del dia px %.2f < %.2f\n", H, M, b.c, lo);
                std::fflush(stdout);
                { char m[120]; std::snprintf(m, sizeof(m), "Donchian: rompe minimo del dia px %.2f", b.c); notify("ASML breakdown", m, false); }
                last_don = b.t;
            }
        }
        ...
        if (in_pos && b.t >= pos_epoch) {
            ...
            // stop; fill pesimista al stop (o al open si abrio por debajo).
                ...
                if ((b.c >= floor_px || EOD_FORCE > 0) && envd("ASML_EOD_FLATTEN", 0) > 0.5) { sold = true; why = "EOD flatten 15:45"; }
            }
            if (sold) {
                std::printf("[%02d:%02d] *** ASML: VENDER *** ~%.2f (%s, entrada %.2f) t=%.0f\n",
                            H, M, exit_px, why, entry, b.t);
                std::fflush(stdout);
                if (audio_gate(true)) { play("sounds/dram_sell.wav", "Hero"); speak("sell A S M L now"); }
                { char m[200]; std::snprintf(m, sizeof(m),
                    "VENDER ASML @ %.2f | %s | entrada %.2f | PnL %+.1f%%",
                    exit_px, why, entry, (exit_px / entry - 1) * 100);
                  notify(why[0] == 'H' ? "ASML: SELL (STOP)" : "ASML: SELL", m, true); }
        ...
        if (pending_buy && !in_pos) {
            ...
            if (rth_entry && (MAX_DAY == 0 || day_entries < (int)MAX_DAY)) {
                if (in_short) {   // reversal: capitulacion confirmada = cubrir corto
                    double px = b.o;
                    std::printf("[%02d:%02d] *** ASML: VENDER PUT *** ~%.2f (reversal a largo, entrada %.2f) t=%.0f\n",
                                H, M, px, s_entry, b.t);
                    std::fflush(stdout);
                    if (audio_gate(true)) { play("sounds/dram_buy.wav", "Glass"); speak("buy A S M L now"); }
                    { char m[200]; std::snprintf(m, sizeof(m),
                        "COMPRAR ASML @ %.2f | reversal a alza | entrada %.2f | mov %+.1f%%",
                ...
                std::printf("[%02d:%02d] *** ASML: COMPRAR *** ~%.2f (capitulacion confirmada; "
                            "target %.2f, floor %.2f) t=%.0f\n", H, M, entry, target_px, floor_px, b.t);
                std::fflush(stdout);
                if (audio_gate(true)) { play("sounds/dram_buy.wav", "Glass"); speak("buy A S M L now"); }
                { char m[200]; std::snprintf(m, sizeof(m),
                    "COMPRAR ASML @ %.2f | target %.2f | floor %.2f | capitulacion confirmada",
        ...
        if (SHORTS > 0) {
            ...
            if (in_short && b.t >= spos_epoch) {
                ...
                // target, manda el STOP (fill al stop o al open si abrio arriba)
                    ...
                    if ((b.c <= s_floor || EOD_FORCE > 0) && envd("ASML_EOD_FLATTEN", 0) > 0.5) { cov = true; why = "EOD cover 15:45"; }
                }
                if (cov) {
                    std::printf("[%02d:%02d] *** ASML: VENDER PUT *** ~%.2f (%s, entrada %.2f) t=%.0f\n",
                                H, M, exit_px, why, s_entry, b.t);
                    std::fflush(stdout);
                    if (audio_gate(true)) { play("sounds/dram_buy.wav", "Ping"); speak("buy A S M L now"); }
                    { char m[200]; std::snprintf(m, sizeof(m),
                        "COMPRAR ASML @ %.2f | %s | entrada %.2f | mov %+.1f%%",
                        exit_px, why, s_entry, (s_entry / exit_px - 1) * 100);
                      notify(why[0] == 'H' ? "ASML: BUY (STOP)" : "ASML: BUY", m, true); }
            ...
            if (pending_short && !in_short && !in_pos) {
                ...
                if (rth_entry && (MAX_DAY == 0 || sday_entries < (int)MAX_DAY)) {
                    ...
                    std::printf("[%02d:%02d] *** ASML: PUT *** ~%.2f (blow-off confirmado; "
                                "target %.2f, floor %.2f) t=%.0f\n", H, M, s_entry, s_target, s_floor, b.t);
                    std::fflush(stdout);
                    if (audio_gate(true)) { play("sounds/dram_sell.wav", "Basso"); speak("sell A S M L now"); }
                    { char m[200]; std::snprintf(m, sizeof(m),
                        "VENDER ASML @ %.2f | target %.2f | floor %.2f | senal de BAJADA (blow-off)",
