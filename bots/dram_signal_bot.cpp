...
static void notify(const char* title, const char* msg, bool urgent) {
    ...
    FILE* f = std::fopen("logs/dram_operations.log", "a");
    if (f) {
...
// unica con prob% (logistica calibrada por backtest; DRAM_V5_A/B).
// DRAM_V5=1 activa; default 0 = byte-identico al motor previo.
static const double V5        = envd("DRAM_V5", 0);
static const double V5_MIN    = envd("DRAM_V5_MIN", 99.0);   // score minimo (de 8)
static const double V5_A      = envd("DRAM_V5_A", -3.2);    // logistica: prob = 1/(1+e^-(A+B*score))
static const double V5_B      = envd("DRAM_V5_B", 0.62);
static const double V5_COOL   = envd("DRAM_V5_COOL", 1800); // s entre señales mismo lado
...
static int v5_opt(char* out, size_t n) {
    FILE* f = fopen("data/options_dram.txt", "r");
...
static void v5_on_bar(const Bar& b, bool alert_hours, int H, int M) {
    ...
    if (sbuy >= V5_MIN && b.t - v5_last_buy >= V5_COOL && sbuy > ssell) {
        ...
        char m[420];
        std::snprintf(m, sizeof(m), "DRAM @ %.2f | prob %.0f%% | %s | %s%s",
                      b.c, p, rb, sc, opt);
        std::printf("[%02d:%02d] *** DRAM V5 BUY *** score %.1f prob %.0f%% %s t=%.0f\n",
                    H, M, sbuy, p, rb, b.t);
        std::fflush(stdout);
        notify("DRAM: BUY", m, true);
    ...
    if (ssell >= V5_MIN && b.t - v5_last_sell >= V5_COOL && ssell > sbuy) {
        ...
        std::snprintf(m, sizeof(m), "DRAM @ %.2f | prob %.0f%% | %s | %s%s",
                      b.c, p, rs, sc, opt);
        std::printf("[%02d:%02d] *** DRAM V5 SELL *** score %.1f prob %.0f%% %s t=%.0f\n",
                    H, M, ssell, p, rs, b.t);
        std::fflush(stdout);
        notify("DRAM: SELL", m, true);
        if (audio_gate(true)) { play("sounds/dram_sell.wav", "Basso"); speak("sell DRAM now"); }
    }
}
// ===== FIN MOTOR v5 =====
...
// clases de señal cerradas con prob% calibrada (data/prob_table_dram.txt,
...
// que atraviesa el nivel = TRAMPA-EVITADA (cancelada + log). DRAM_V6_RETEST=0
// restaura el disparo inmediato v6.0.
#include <sys/stat.h>
#include <algorithm>
#include <filesystem>

static const double V6_ON       = envd("DRAM_V6", 1);           // 1 = activo
static const double V6_MIN      = envd("DRAM_V6_MIN", 6.0);     // score minimo (de 10)
static const double V6_COOL     = envd("DRAM_V6_COOL", 1800);   // s entre señales mismo lado
static const double V6_PROB_MIN = envd("DRAM_V6_PROB_MIN", 55.0);
static const double V6_MAX_CLASS_DAY = envd("DRAM_V6_MAX_CLASS_DAY", 2);
static const double V6_RVOL     = envd("DRAM_V6_RVOL", 1.5);
static const double V6_RVOL_RECLAIM = envd("DRAM_V6_RVOL_RECLAIM", 2.0);
static const double V6_RETR_CONT = envd("DRAM_V6_RETR_CONT", 0.50);
static const double V6_RETR_REV  = envd("DRAM_V6_RETR_REV", 0.62);
static const double V6_SQUEEZE_PCT = envd("DRAM_V6_SQUEEZE_PCT", 10.0);
static const double V6_ADX_TREND = envd("DRAM_V6_ADX_TREND", 25.0);
static const double V6_ADX_RANGE = envd("DRAM_V6_ADX_RANGE", 20.0);
static const double V6_OR_LATERAL = envd("DRAM_V6_OR_LATERAL", 1.2);
static const double V6_GAP_BREAKAWAY = envd("DRAM_V6_GAP_BREAKAWAY", 1.0);
static const double V6_KRX      = envd("DRAM_KRX", 0);          // 1 = sesion KST (open min 540)
static const double V6_DEBUG    = envd("DRAM_V6_DEBUG", 0);
static const double V6_RETEST   = envd("DRAM_V6_RETEST", 1);    // v6.1: 1 = retest-confirm en rupturas
static const double V6_RETEST_MAX = envd("DRAM_V6_RETEST_MAX", 20); // bars 1m armada antes de expirar
...
// Override CSV: export DRAM_V6_RETEST_EXEMPT="VWAP_LOSS_SHORT,ORB_SHORT"
...
static const bool v6_retest_exempt_init = []() {
    const char* v = std::getenv("DRAM_V6_RETEST_EXEMPT");
...
static void v6_emit(bool isbuy, int cls, double prob, double score, const char* r,
    ...
    v6_fires_today[cls]++;
    std::printf("[%02d:%02d] *** DRAM V6 %s *** prob %.0f%% clase %s score %.1f %s t=%.0f\n",
                H, M, isbuy ? "BUY" : "SELL", prob, V6_CLS[cls], score, r, b.t);
    std::fflush(stdout);
    char m[420];
    std::snprintf(m, sizeof(m), "DRAM @ %.2f | prob %.0f%% | %s | fase=%s",
                  b.c, prob, r, V6_PHASE_NAME[ph]);
    notify(isbuy ? "DRAM: BUY" : "DRAM: SELL", m, true);
    ...
    if (audio_gate(true)) {
        if (isbuy) { play("sounds/dram_buy.wav", "Glass");
            char sp[120]; std::snprintf(sp, sizeof(sp),
                "buy DRAM now, probability %.0f percent", prob); speak(sp); }
        else { play("sounds/dram_sell.wav", "Basso");
            char sp[120]; std::snprintf(sp, sizeof(sp),
                "sell DRAM now, probability %.0f percent", prob); speak(sp); } } }
...
static void v6_on_bar(const Bar& b, bool alert_hours, int H, int M) {
    ...
    v6_prob.maybe_reload("data/prob_table_dram.txt", b.t);
    ...
    auto v6_retest_step = [&](V6Armed& a, double& blk, bool veto_now) {
        ...
        if (cross > 0 && cross > 0.5 * imp) {
            std::printf("[%02d:%02d] DRAM V6 TRAMPA-EVITADA clase %s nivel %.2f px %.2f "
                        "impulso %.2f t=%.0f\n",
...
int main(int argc, char** argv) {
    ...
    FILE* in = stdin;
    if (!use_stdin) {
        // daemon IBKR escribe data/bars_dram_ibkr.txt; tail -F lo sigue 
        // (Yunior 2026-07-20: only ibkr — fuente unica)
        in = popen("tail -n +1 -F data/bars_dram_ibkr.txt 2>>logs/bridge_dram.log", "r");
    ...
    while (std::fgets(line, sizeof(line), in)) {
        ...
        if (in_pos && b.t >= pos_epoch) {
            ...
            // stop; fill pesimista al stop (o al open si abrio por debajo).
                ...
                if ((b.c >= floor_px || EOD_FORCE > 0) && envd("DRAM_EOD_FLATTEN", 0) > 0.5) { sold = true; why = "EOD flatten 15:45"; }
            }
            if (sold) {
                ...
                { char m[200]; std::snprintf(m, sizeof(m),
                  ...
                  notify(why[0] == 'H' ? "DRAM: SELL (STOP)" : "DRAM: SELL", m, true); }
        ...
        if (pending_buy && !in_pos) {
            ...
            if (rth_entry && (MAX_DAY == 0 || day_entries < (int)MAX_DAY)) {
                if (in_short) {   // reversal: capitulacion confirmada = cubrir corto
                    ...
                    if (audio_gate(true)) { play("sounds/dram_buy.wav", "Glass"); speak("buy DRAM now"); }
                    { char m[200]; std::snprintf(m, sizeof(m),
                        "COMPRAR DRAM @ %.2f | reversal a alza | entrada %.2f | mov %+.1f%%",
                ...
                if (audio_gate(true)) { play("sounds/dram_buy.wav", "Glass"); speak("buy DRAM now"); }
                { char m[200]; std::snprintf(m, sizeof(m),
                    "COMPRAR DRAM @ %.2f | target %.2f | floor %.2f | capitulacion confirmada",
        ...
        if (SHORTS > 0) {
            ...
            if (in_short && b.t >= spos_epoch) {
                ...
                // target, manda el STOP (fill al stop o al open si abrio arriba)
                    ...
                    if ((b.c <= s_floor || EOD_FORCE > 0) && envd("DRAM_EOD_FLATTEN", 0) > 0.5) { cov = true; why = "EOD cover 15:45"; }
                }
                if (cov) {
                    std::printf("[%02d:%02d] *** DRAM: VENDER PUT *** ~%.2f (%s, entrada %.2f) t=%.0f\n",
                                H, M, exit_px, why, s_entry, b.t);
                    std::fflush(stdout);
                    if (audio_gate(true)) { play("sounds/dram_buy.wav", "Ping"); speak("buy DRAM now"); }
                    { char m[200]; std::snprintf(m, sizeof(m),
                        "COMPRAR DRAM @ %.2f | %s | entrada %.2f | mov %+.1f%%",
                        exit_px, why, s_entry, (s_entry / exit_px - 1) * 100);
                      notify(why[0] == 'H' ? "DRAM: BUY (STOP)" : "DRAM: BUY", m, true); }
            ...
            if (pending_short && !in_short && !in_pos) {
                ...
                if (rth_entry && (MAX_DAY == 0 || sday_entries < (int)MAX_DAY)) {
                    ...
                    if (audio_gate(true)) { play("sounds/dram_sell.wav", "Basso"); speak("sell DRAM now"); }
                    { char m[200]; std::snprintf(m, sizeof(m),
                        "VENDER DRAM @ %.2f | target %.2f | floor %.2f | senal de BAJADA (blow-off)",
