// spy_signal_bot.cpp — SPY buy/sell signal engine, pure C++
// ============================================================
...
//
// build: clang++ -std=c++17 -O2 -o spy_signal_bot spy_signal_bot.cpp
// run:   ./spy_signal_bot            (spawns bridge)
//        ./spy_signal_bot --stdin    (replay: feed bar lines for testing)
...
// ---- params: por-ticker via env SPY_* (orden Yunior 2026-07-10 "tune per
...
static const int    BB_N = 20;      static const double BB_STD = envd("SPY_BB_STD", 3.0);
static const int    RSI_N = 14;     static const double RSI_OS = envd("SPY_RSI_OS", 25.0);
static const int    VOL_N = 20;     static const double VOL_MULT = envd("SPY_VOL_MULT", 1.2);
static const int    ATR_N = 14;
static const int    CONFIRM_WINDOW = 60;
static const double TARGET_PCT = envd("SPY_TARGET", 4.0),
                    FLOOR_PCT  = envd("SPY_FLOOR", 1.0),
                    TRAIL_ATR  = envd("SPY_TRAIL_ATR", 3.0);
// HARD STOP alert (orden 2026-07-10): a -SPY_STOP% del entry el bot GRITA la
// perdida (SELL-STOP) en vez de callarse a esperar el floor. Humano decide.
static const double STOP_PCT = envd("SPY_STOP", 3.0);
...
static const double SKIP_OPEN      = envd("SPY_SKIP_OPEN", 0);
static const double TIME_STOP_MIN  = envd("SPY_TIME_STOP_MIN", 0);
static const double EOD_FORCE      = envd("SPY_EOD_FORCE", 0);
static const double CONFIRM_STRICT = envd("SPY_CONFIRM_STRICT", 0);
static const double MAX_DAY        = envd("SPY_MAX_DAY", 0);
...
static const double SCORE_MIN  = envd("SPY_SCORE_MIN", 0);
static const double WHALE_USD  = envd("SPY_WHALE_USD", 75000);
...
static const double SPREAD_MAX = envd("SPY_SPREAD_MAX", 0);
// TREND MODE generico (SPY_MODE=trend): flip Supertrend 5m / ruptura max del
// dia con CUSUM >= TREND_CUSUM; TREND_VWAP=1 exige ademas c>VWAP y vol>=volMA.
static const bool TREND_MODE = [] {
    const char* v = std::getenv("SPY_MODE");
    return v && !std::strcmp(v, "trend");
}();
static const double TREND_CUSUM = envd("SPY_TREND_CUSUM", 0.01);
static const double TREND_VWAP  = envd("SPY_TREND_VWAP", 0);
// ===== v4 AMBAS DIRECCIONES (orden Yunior 2026-07-11: señales cuando sube
// Y cuando baja). SPY_SHORTS=1 activa el espejo corto: blow-off arriba de la
...
static const double SHORTS = envd("SPY_SHORTS", 0);
// exits propios del corto (los mercados caen distinto a como suben);
// sin definir heredan los del largo
static const double S_TARGET = envd("SPY_S_TARGET", TARGET_PCT);
static const double S_STOP   = envd("SPY_S_STOP", STOP_PCT);
static const double S_TRAIL  = envd("SPY_S_TRAIL", TRAIL_ATR);
static const double S_FLOOR  = envd("SPY_S_FLOOR", FLOOR_PCT);
static const double S_TSTOP  = envd("SPY_S_TSTOP", TIME_STOP_MIN);
...
static const double S_BB_STD    = envd("SPY_S_BB_STD", BB_STD);
static const double S_RSI_OS    = envd("SPY_S_RSI_OS", RSI_OS);
static const double S_VOL_MULT  = envd("SPY_S_VOL_MULT", VOL_MULT);
static const double S_SCORE_MIN = envd("SPY_S_SCORE_MIN", SCORE_MIN);
static const double S_TCUSUM    = envd("SPY_S_TREND_CUSUM", TREND_CUSUM);
...
static const double CANDLE   = envd("SPY_CANDLE", 0);
static const double S_CANDLE = envd("SPY_S_CANDLE", CANDLE);
...
static const double QUAKE_MIN    = envd("SPY_QUAKE_MIN", 0.02);
static const double QUAKE_BANNER = envd("SPY_QUAKE_BANNER", 0);
static const bool S_MODE_TREND = [] {
    const char* v = std::getenv("SPY_S_MODE");
    if (v && !std::strcmp(v, "trend")) return true;
    if (v && !std::strcmp(v, "mr")) return false;
    const char* m = std::getenv("SPY_MODE");           // hereda el modo largo
    return m && !std::strcmp(m, "trend");
}();
static const char* SPOS_FILE = "data/pos_spy_s.txt";
...
static const char* WHALE_FILE = "data/whale_spy.txt";
static const char* NBBO_FILE  = "data/nbbo_spy.txt";
...
static const char* POS_FILE = "data/pos_spy.txt";
...
static void notify(const char* title, const char* msg, bool urgent) {
    ...
    FILE* f = std::fopen("logs/spy_operations.log", "a");
    if (f) {
...
// unica con prob% (logistica calibrada por backtest; SPY_V5_A/B).
// SPY_V5=1 activa; default 0 = byte-identico al motor previo.
static const double V5        = envd("SPY_V5", 0);
static const double V5_MIN    = envd("SPY_V5_MIN", 99.0);   // score minimo (de 8)
static const double V5_A      = envd("SPY_V5_A", -3.2);    // logistica: prob = 1/(1+e^-(A+B*score))
static const double V5_B      = envd("SPY_V5_B", 0.62);
static const double V5_COOL   = envd("SPY_V5_COOL", 1800); // s entre señales mismo lado
...
static int v5_opt(char* out, size_t n) {
    FILE* f = fopen("data/options_spy.txt", "r");
...
static void v5_on_bar(const Bar& b, bool alert_hours, int H, int M) {
    ...
    if (sbuy >= V5_MIN && b.t - v5_last_buy >= V5_COOL && sbuy > ssell) {
        ...
        char m[420];
        std::snprintf(m, sizeof(m), "SPY @ %.2f | prob %.0f%% | %s | %s%s",
                      b.c, p, rb, sc, opt);
        std::printf("[%02d:%02d] *** SPY V5 BUY *** score %.1f prob %.0f%% %s t=%.0f\n",
                    H, M, sbuy, p, rb, b.t);
        std::fflush(stdout);
        notify("SPY: BUY", m, true);
        if (audio_gate(true)) { play("sounds/dram_buy.wav", "Glass"); speak("buy S and P five hundred now"); }
    }
    if (ssell >= V5_MIN && b.t - v5_last_sell >= V5_COOL && ssell > sbuy) {
        ...
        std::snprintf(m, sizeof(m), "SPY @ %.2f | prob %.0f%% | %s | %s%s",
                      b.c, p, rs, sc, opt);
        std::printf("[%02d:%02d] *** SPY V5 SELL *** score %.1f prob %.0f%% %s t=%.0f\n",
                    H, M, ssell, p, rs, b.t);
        std::fflush(stdout);
        notify("SPY: SELL", m, true);
        if (audio_gate(true)) { play("sounds/dram_sell.wav", "Basso"); speak("sell S and P five hundred now"); }
    }
}
// ===== FIN MOTOR v5 =====
...
// clases de señal cerradas con prob% calibrada (data/prob_table_spy.txt,
...
// que atraviesa el nivel = TRAMPA-EVITADA (cancelada + log). SPY_V6_RETEST=0
// restaura el disparo inmediato v6.0.
#include <sys/stat.h>
#include <algorithm>
#include <filesystem>

static const double V6_ON       = envd("SPY_V6", 1);           // 1 = activo
static const double V6_MIN      = envd("SPY_V6_MIN", 6.0);     // score minimo (de 10)
static const double V6_COOL     = envd("SPY_V6_COOL", 1800);   // s entre señales mismo lado
static const double V6_PROB_MIN = envd("SPY_V6_PROB_MIN", 55.0);
static const double V6_MAX_CLASS_DAY = envd("SPY_V6_MAX_CLASS_DAY", 2);
static const double V6_RVOL     = envd("SPY_V6_RVOL", 1.5);
static const double V6_RVOL_RECLAIM = envd("SPY_V6_RVOL_RECLAIM", 2.0);
static const double V6_RETR_CONT = envd("SPY_V6_RETR_CONT", 0.50);
static const double V6_RETR_REV  = envd("SPY_V6_RETR_REV", 0.62);
static const double V6_SQUEEZE_PCT = envd("SPY_V6_SQUEEZE_PCT", 10.0);
static const double V6_ADX_TREND = envd("SPY_V6_ADX_TREND", 25.0);
static const double V6_ADX_RANGE = envd("SPY_V6_ADX_RANGE", 20.0);
static const double V6_OR_LATERAL = envd("SPY_V6_OR_LATERAL", 1.2);
static const double V6_GAP_BREAKAWAY = envd("SPY_V6_GAP_BREAKAWAY", 1.0);
static const double V6_KRX      = envd("SPY_KRX", 0);          // 1 = sesion KST (open min 540)
static const double V6_DEBUG    = envd("SPY_V6_DEBUG", 0);
static const double V6_RETEST   = envd("SPY_V6_RETEST", 1);    // v6.1: 1 = retest-confirm en rupturas
static const double V6_RETEST_MAX = envd("SPY_V6_RETEST_MAX", 20); // bars 1m armada antes de expirar
...
// Override CSV: export SPY_V6_RETEST_EXEMPT="VWAP_LOSS_SHORT,ORB_SHORT"
...
static const bool v6_retest_exempt_init = []() {
    const char* v = std::getenv("SPY_V6_RETEST_EXEMPT");
...
static void v6_emit(bool isbuy, int cls, double prob, double score, const char* r,
    ...
    v6_fires_today[cls]++;
    std::printf("[%02d:%02d] *** SPY V6 %s *** prob %.0f%% clase %s score %.1f %s t=%.0f\n",
                H, M, isbuy ? "BUY" : "SELL", prob, V6_CLS[cls], score, r, b.t);
    std::fflush(stdout);
    char m[420];
    std::snprintf(m, sizeof(m), "SPY @ %.2f | prob %.0f%% | %s | fase=%s",
                  b.c, prob, r, V6_PHASE_NAME[ph]);
    notify(isbuy ? "SPY: BUY" : "SPY: SELL", m, true);
    ...
    if (audio_gate(true)) {
        if (isbuy) { play("sounds/dram_buy.wav", "Glass");
            char sp[120]; std::snprintf(sp, sizeof(sp),
                "buy S and P five hundred now, probability %.0f percent", prob); speak(sp); }
        else { play("sounds/dram_sell.wav", "Basso");
            char sp[120]; std::snprintf(sp, sizeof(sp),
                "sell S and P five hundred now, probability %.0f percent", prob); speak(sp); } } }
...
static void v6_on_bar(const Bar& b, bool alert_hours, int H, int M) {
    ...
    v6_prob.maybe_reload("data/prob_table_spy.txt", b.t);
    ...
    auto v6_retest_step = [&](V6Armed& a, double& blk, bool veto_now) {
        ...
        if (cross > 0 && cross > 0.5 * imp) {
            std::printf("[%02d:%02d] SPY V6 TRAMPA-EVITADA clase %s nivel %.2f px %.2f "
                        "impulso %.2f t=%.0f\n",
...
int main(int argc, char** argv) {
    ...
    FILE* in = stdin;
    if (!use_stdin) {
        // daemon IBKR escribe data/bars_spy_ibkr.txt; tail -F lo sigue 
        // (Yunior 2026-07-20: only ibkr — fuente unica)
        in = popen("tail -n +1 -F data/bars_spy_ibkr.txt 2>>logs/bridge_spy.log", "r");
        if (!in) { std::fprintf(stderr, "no bridge\n"); return 1; }
        std::fprintf(stderr, "spy_signal_bot (C++): bridge SPY 1m real iniciado\n");
    }
    ...
    while (std::fgets(line, sizeof(line), in)) {
        ...
        if (nbars > 1 && prev_close > 0) { /* prev_close ya actualizado: usar retorno del bar */ }
        {
            ...
            if (last_c_for_ret > 0) {
                ...
                if (alert_hours && vol_ok_radar && b.t - last_cusum > 3600) {
                    if (cusum_up > hthr) {
                        std::printf("[%02d:%02d] CUSUM: SPY SUBIENDO fuerte (+%.2f%% acumulado) px %.2f t=%.0f\n",
                                    H, M, cusum_up * 100, b.c, b.t);
                        std::fflush(stdout);
                        { char m[160]; std::snprintf(m, sizeof(m), "CUSUM: subiendo fuerte %+.2f%% px %.2f", cusum_up*100, b.c); notify("SPY TERREMOTO ALZA", m, QUAKE_BANNER > 0); }
                        cusum_up = 0; cusum_dn = 0; last_cusum = b.t;
                    } else if (cusum_dn < -hthr) {
                        std::printf("[%02d:%02d] CUSUM: SPY CAYENDO fuerte (%.2f%% acumulado) px %.2f t=%.0f\n",
                                    H, M, cusum_dn * 100, b.c, b.t);
                        std::fflush(stdout);
                        { char m[160]; std::snprintf(m, sizeof(m), "CUSUM: cayendo fuerte %.2f%% px %.2f", cusum_dn*100, b.c); notify("SPY TERREMOTO CAIDA", m, QUAKE_BANNER > 0); }
                        cusum_up = 0; cusum_dn = 0; last_cusum = b.t;
                    }
                }
            }
        ...
        if (bar5 && atr5 > 0 && n5 > 10) {
            ...
            if (st_trend != 0 && nt != st_trend && alert_hours && b.t - last_st > 3600) {
                if (nt > 0) {
                    std::printf("[%02d:%02d] SUPERTREND: tendencia SPY cambio a ALCISTA px %.2f\n", H, M, b.c);
                    { char m[120]; std::snprintf(m, sizeof(m), "Supertrend: tendencia ALCISTA px %.2f", b.c); notify("SPY tendencia", m, false); }
                } else {
                    std::printf("[%02d:%02d] SUPERTREND: tendencia SPY cambio a BAJISTA px %.2f\n", H, M, b.c);
                    { char m[120]; std::snprintf(m, sizeof(m), "Supertrend: tendencia BAJISTA px %.2f", b.c); notify("SPY tendencia", m, false); }
                }
                std::fflush(stdout);
                last_st = b.t;
        ...
        if ((int)dh20.size() == 390 && alert_hours && b.t - last_don > 3600) {
            ...
            if (b.c > hi) {
                std::printf("[%02d:%02d] DONCHIAN: SPY rompe maximo del dia px %.2f > %.2f\n", H, M, b.c, hi);
                std::fflush(stdout);
                { char m[120]; std::snprintf(m, sizeof(m), "Donchian: rompe maximo del dia px %.2f", b.c); notify("SPY breakout", m, false); }
                last_don = b.t;
            } else if (b.c < lo) {
                std::printf("[%02d:%02d] DONCHIAN: SPY rompe minimo del dia px %.2f < %.2f\n", H, M, b.c, lo);
                std::fflush(stdout);
                { char m[120]; std::snprintf(m, sizeof(m), "Donchian: rompe minimo del dia px %.2f", b.c); notify("SPY breakdown", m, false); }
                last_don = b.t;
            }
        }
        ...
        if (in_pos && b.t >= pos_epoch) {
            ...
            // stop; fill pesimista al stop (o al open si abrio por debajo).
                ...
                if ((b.c >= floor_px || EOD_FORCE > 0) && envd("SPY_EOD_FLATTEN", 0) > 0.5) { sold = true; why = "EOD flatten 15:45"; }
            }
            if (sold) {
                std::printf("[%02d:%02d] *** SPY: VENDER *** ~%.2f (%s, entrada %.2f) t=%.0f\n",
                            H, M, exit_px, why, entry, b.t);
                std::fflush(stdout);
                if (audio_gate(true)) { play("sounds/dram_sell.wav", "Hero"); speak("sell S and P five hundred now"); }
                { char m[200]; std::snprintf(m, sizeof(m),
                    "VENDER SPY @ %.2f | %s | entrada %.2f | PnL %+.1f%%",
                    exit_px, why, entry, (exit_px / entry - 1) * 100);
                  notify(why[0] == 'H' ? "SPY: SELL (STOP)" : "SPY: SELL", m, true); }
        ...
        if (pending_buy && !in_pos) {
            ...
            if (rth_entry && (MAX_DAY == 0 || day_entries < (int)MAX_DAY)) {
                if (in_short) {   // reversal: capitulacion confirmada = cubrir corto
                    double px = b.o;
                    std::printf("[%02d:%02d] *** SPY: VENDER PUT *** ~%.2f (reversal a largo, entrada %.2f) t=%.0f\n",
                                H, M, px, s_entry, b.t);
                    std::fflush(stdout);
                    if (audio_gate(true)) { play("sounds/dram_buy.wav", "Glass"); speak("buy S and P five hundred now"); }
                    { char m[200]; std::snprintf(m, sizeof(m),
                        "COMPRAR SPY @ %.2f | reversal a alza | entrada %.2f | mov %+.1f%%",
                ...
                std::printf("[%02d:%02d] *** SPY: COMPRAR *** ~%.2f (capitulacion confirmada; "
                            "target %.2f, floor %.2f) t=%.0f\n", H, M, entry, target_px, floor_px, b.t);
                std::fflush(stdout);
                if (audio_gate(true)) { play("sounds/dram_buy.wav", "Glass"); speak("buy S and P five hundred now"); }
                { char m[200]; std::snprintf(m, sizeof(m),
                    "COMPRAR SPY @ %.2f | target %.2f | floor %.2f | capitulacion confirmada",
        ...
        if (SHORTS > 0) {
            ...
            if (in_short && b.t >= spos_epoch) {
                ...
                // target, manda el STOP (fill al stop o al open si abrio arriba)
                    ...
                    if ((b.c <= s_floor || EOD_FORCE > 0) && envd("SPY_EOD_FLATTEN", 0) > 0.5) { cov = true; why = "EOD cover 15:45"; }
                }
                if (cov) {
                    std::printf("[%02d:%02d] *** SPY: VENDER PUT *** ~%.2f (%s, entrada %.2f) t=%.0f\n",
                                H, M, exit_px, why, s_entry, b.t);
                    std::fflush(stdout);
                    if (audio_gate(true)) { play("sounds/dram_buy.wav", "Ping"); speak("buy S and P five hundred now"); }
                    { char m[200]; std::snprintf(m, sizeof(m),
                        "COMPRAR SPY @ %.2f | %s | entrada %.2f | mov %+.1f%%",
                        exit_px, why, s_entry, (s_entry / exit_px - 1) * 100);
                      notify(why[0] == 'H' ? "SPY: BUY (STOP)" : "SPY: BUY", m, true); }
            ...
            if (pending_short && !in_short && !in_pos) {
                ...
                if (rth_entry && (MAX_DAY == 0 || sday_entries < (int)MAX_DAY)) {
                    ...
                    std::printf("[%02d:%02d] *** SPY: PUT *** ~%.2f (blow-off confirmado; "
                                "target %.2f, floor %.2f) t=%.0f\n", H, M, s_entry, s_target, s_floor, b.t);
                    std::fflush(stdout);
                    if (audio_gate(true)) { play("sounds/dram_sell.wav", "Basso"); speak("sell S and P five hundred now"); }
                    { char m[200]; std::snprintf(m, sizeof(m),
                        "VENDER SPY @ %.2f | target %.2f | floor %.2f | senal de BAJADA (blow-off)",
