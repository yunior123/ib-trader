...
static const double SCORE_MIN  = envd("NOK_SCORE_MIN", 0);
static const double WHALE_USD  = envd("NOK_WHALE_USD", 75000);
...
static const double SPREAD_MAX = envd("NOK_SPREAD_MAX", 0);
// TREND MODE generico (NOK_MODE=trend): flip Supertrend 5m / ruptura max del
// dia con CUSUM >= TREND_CUSUM; TREND_VWAP=1 exige ademas c>VWAP y vol>=volMA.
static const bool TREND_MODE = [] {
    const char* v = std::getenv("NOK_MODE");
    return v && !std::strcmp(v, "trend");
}();
static const double TREND_CUSUM = envd("NOK_TREND_CUSUM", 0.01);
static const double TREND_VWAP  = envd("NOK_TREND_VWAP", 0);
// ===== v4 AMBAS DIRECCIONES (orden Yunior 2026-07-11: señales cuando sube
// Y cuando baja). NOK_SHORTS=1 activa el espejo corto: blow-off arriba de la
...
static const double SHORTS = envd("NOK_SHORTS", 0);
// exits propios del corto (los mercados caen distinto a como suben);
// sin definir heredan los del largo
static const double S_TARGET = envd("NOK_S_TARGET", TARGET_PCT);
static const double S_STOP   = envd("NOK_S_STOP", STOP_PCT);
static const double S_TRAIL  = envd("NOK_S_TRAIL", TRAIL_ATR);
static const double S_FLOOR  = envd("NOK_S_FLOOR", FLOOR_PCT);
static const double S_TSTOP  = envd("NOK_S_TSTOP", TIME_STOP_MIN);
...
static const double S_BB_STD    = envd("NOK_S_BB_STD", BB_STD);
static const double S_RSI_OS    = envd("NOK_S_RSI_OS", RSI_OS);
...
static const double CANDLE   = envd("NOK_CANDLE", 0);
static const double S_CANDLE = envd("NOK_S_CANDLE", CANDLE);
...
static const bool S_MODE_TREND = [] {
    ...
    const char* m = std::getenv("NOK_MODE");           // hereda el modo largo
    return m && !std::strcmp(m, "trend");
}();
static const char* SPOS_FILE = "data/pos_nok_s.txt";
...
static const char* WHALE_FILE = "data/whale_nok.txt";
static const char* NBBO_FILE  = "data/nbbo_nok.txt";
...
static void notify(const char* title, const char* msg, bool urgent) {
    ...
    FILE* f = std::fopen("logs/nok_operations.log", "a");
    if (f) {
...
// unica con prob% (logistica calibrada por backtest; NOK_V5_A/B).
// NOK_V5=1 activa; default 0 = byte-identico al motor previo.
static const double V5        = envd("NOK_V5", 0);
static const double V5_MIN    = envd("NOK_V5_MIN", 99.0);   // score minimo (de 8)
static const double V5_A      = envd("NOK_V5_A", -3.2);    // logistica: prob = 1/(1+e^-(A+B*score))
static const double V5_B      = envd("NOK_V5_B", 0.62);
static const double V5_COOL   = envd("NOK_V5_COOL", 1800); // s entre señales mismo lado
...
static int v5_opt(char* out, size_t n) {
    FILE* f = fopen("data/options_nok.txt", "r");
...
static void v5_on_bar(const Bar& b, bool alert_hours, int H, int M) {
    ...
    if (sbuy >= V5_MIN && b.t - v5_last_buy >= V5_COOL && sbuy > ssell) {
        ...
        char m[420];
        std::snprintf(m, sizeof(m), "NOK @ %.2f | prob %.0f%% | %s | %s%s",
                      b.c, p, rb, sc, opt);
        std::printf("[%02d:%02d] *** NOK V5 BUY *** score %.1f prob %.0f%% %s t=%.0f\n",
                    H, M, sbuy, p, rb, b.t);
        std::fflush(stdout);
        notify("NOK: BUY", m, true);
    ...
    if (ssell >= V5_MIN && b.t - v5_last_sell >= V5_COOL && ssell > sbuy) {
        ...
        std::snprintf(m, sizeof(m), "NOK @ %.2f | prob %.0f%% | %s | %s%s",
                      b.c, p, rs, sc, opt);
        std::printf("[%02d:%02d] *** NOK V5 SELL *** score %.1f prob %.0f%% %s t=%.0f\n",
                    H, M, ssell, p, rs, b.t);
        std::fflush(stdout);
        notify("NOK: SELL", m, true);
        if (audio_gate(true)) { play("sounds/dram_sell.wav", "Basso"); speak("sell Nokia now"); }
    }
}
// ===== FIN MOTOR v5 =====
...
// clases de señal cerradas con prob% calibrada (data/prob_table_nok.txt,
...
// que atraviesa el nivel = TRAMPA-EVITADA (cancelada + log). NOK_V6_RETEST=0
// restaura el disparo inmediato v6.0.
#include <sys/stat.h>
#include <algorithm>
#include <filesystem>

static const double V6_ON       = envd("NOK_V6", 1);           // 1 = activo
static const double V6_MIN      = envd("NOK_V6_MIN", 6.0);     // score minimo (de 10)
static const double V6_COOL     = envd("NOK_V6_COOL", 1800);   // s entre señales mismo lado
static const double V6_PROB_MIN = envd("NOK_V6_PROB_MIN", 55.0);
static const double V6_MAX_CLASS_DAY = envd("NOK_V6_MAX_CLASS_DAY", 2);
static const double V6_RVOL     = envd("NOK_V6_RVOL", 1.5);
static const double V6_RVOL_RECLAIM = envd("NOK_V6_RVOL_RECLAIM", 2.0);
static const double V6_RETR_CONT = envd("NOK_V6_RETR_CONT", 0.50);
static const double V6_RETR_REV  = envd("NOK_V6_RETR_REV", 0.62);
static const double V6_SQUEEZE_PCT = envd("NOK_V6_SQUEEZE_PCT", 10.0);
static const double V6_ADX_TREND = envd("NOK_V6_ADX_TREND", 25.0);
static const double V6_ADX_RANGE = envd("NOK_V6_ADX_RANGE", 20.0);
static const double V6_OR_LATERAL = envd("NOK_V6_OR_LATERAL", 1.2);
static const double V6_GAP_BREAKAWAY = envd("NOK_V6_GAP_BREAKAWAY", 1.0);
static const double V6_KRX      = envd("NOK_KRX", 0);          // 1 = sesion KST (open min 540)
static const double V6_DEBUG    = envd("NOK_V6_DEBUG", 0);
static const double V6_RETEST   = envd("NOK_V6_RETEST", 1);    // v6.1: 1 = retest-confirm en rupturas
static const double V6_RETEST_MAX = envd("NOK_V6_RETEST_MAX", 20); // bars 1m armada antes de expirar
...
// Override CSV: export NOK_V6_RETEST_EXEMPT="VWAP_LOSS_SHORT,ORB_SHORT"
...
static const bool v6_retest_exempt_init = []() {
    const char* v = std::getenv("NOK_V6_RETEST_EXEMPT");
...
static void v6_emit(bool isbuy, int cls, double prob, double score, const char* r,
    ...
    v6_fires_today[cls]++;
    std::printf("[%02d:%02d] *** NOK V6 %s *** prob %.0f%% clase %s score %.1f %s t=%.0f\n",
                H, M, isbuy ? "BUY" : "SELL", prob, V6_CLS[cls], score, r, b.t);
    std::fflush(stdout);
    char m[420];
    std::snprintf(m, sizeof(m), "NOK @ %.2f | prob %.0f%% | %s | fase=%s",
                  b.c, prob, r, V6_PHASE_NAME[ph]);
    notify(isbuy ? "NOK: BUY" : "NOK: SELL", m, true);
    ...
    if (audio_gate(true)) {
        if (isbuy) { play("sounds/dram_buy.wav", "Glass");
            char sp[120]; std::snprintf(sp, sizeof(sp),
                "buy Nokia now, probability %.0f percent", prob); speak(sp); }
        else { play("sounds/dram_sell.wav", "Basso");
            char sp[120]; std::snprintf(sp, sizeof(sp),
                "sell Nokia now, probability %.0f percent", prob); speak(sp); } } }
...
static void v6_on_bar(const Bar& b, bool alert_hours, int H, int M) {
    ...
    v6_prob.maybe_reload("data/prob_table_nok.txt", b.t);
    ...
    auto v6_retest_step = [&](V6Armed& a, double& blk, bool veto_now) {
        ...
        if (cross > 0 && cross > 0.5 * imp) {
            std::printf("[%02d:%02d] NOK V6 TRAMPA-EVITADA clase %s nivel %.2f px %.2f "
                        "impulso %.2f t=%.0f\n",
...
int main(int argc, char** argv) {
    ...
    if (!use_stdin) {
        ...
        in = popen("tail -n +1 -F data/bars_nok_ibkr.txt 2>>logs/bridge_nok.log", "r");
    ...
    while (std::fgets(line, sizeof(line), in)) {
        ...
        if (in_pos && b.t >= pos_epoch) {
            ...
            // stop; fill pesimista al stop (o al open si abrio por debajo).
                ...
                if ((b.c >= floor_px || EOD_FORCE > 0) && envd("NOK_EOD_FLATTEN", 0) > 0.5) { sold = true; why = "EOD flatten 15:45"; }
            }
            if (sold) {
                std::printf("[%02d:%02d] *** NOK: VENDER *** ~%.2f (%s, entrada %.2f) t=%.0f\n",
                ...
                { char m[200]; std::snprintf(m, sizeof(m),
                  ...
                  notify(why[0] == 'H' ? "NOK: SELL (STOP)" : "NOK: SELL", m, true); }
        ...
        if (pending_buy && !in_pos) {
            ...
            if (rth_entry && (MAX_DAY == 0 || day_entries < (int)MAX_DAY)) {
                if (in_short) {   // reversal: capitulacion confirmada = cubrir corto
                    double px = b.o;
                    std::printf("[%02d:%02d] *** NOK: VENDER PUT *** ~%.2f (reversal a largo, entrada %.2f) t=%.0f\n",
                                H, M, px, s_entry, b.t);
                    std::fflush(stdout);
                    if (audio_gate(true)) { play("sounds/dram_buy.wav", "Glass"); speak("buy Nokia now"); }
                    { char m[200]; std::snprintf(m, sizeof(m),
                        "COMPRAR NOK @ %.2f | reversal a alza | entrada %.2f | mov %+.1f%%",
                ...
                if (audio_gate(true)) { play("sounds/dram_buy.wav", "Glass"); speak("buy Nokia now"); }
                { char m[200]; std::snprintf(m, sizeof(m),
                    "COMPRAR NOK @ %.2f | target %.2f | floor %.2f | capitulacion confirmada",
        ...
        if (SHORTS > 0) {
            ...
            if (in_short && b.t >= spos_epoch) {
                ...
                // target, manda el STOP (fill al stop o al open si abrio arriba)
                    ...
                    if ((b.c <= s_floor || EOD_FORCE > 0) && envd("NOK_EOD_FLATTEN", 0) > 0.5) { cov = true; why = "EOD cover 15:45"; }
                }
                if (cov) {
                    std::printf("[%02d:%02d] *** NOK: VENDER PUT *** ~%.2f (%s, entrada %.2f) t=%.0f\n",
                                H, M, exit_px, why, s_entry, b.t);
                    std::fflush(stdout);
                    if (audio_gate(true)) { play("sounds/dram_buy.wav", "Ping"); speak("buy Nokia now"); }
                    { char m[200]; std::snprintf(m, sizeof(m),
                        "COMPRAR NOK @ %.2f | %s | entrada %.2f | mov %+.1f%%",
                        exit_px, why, s_entry, (s_entry / exit_px - 1) * 100);
                      notify(why[0] == 'H' ? "NOK: BUY (STOP)" : "NOK: BUY", m, true); }
            ...
            if (pending_short && !in_short && !in_pos) {
                ...
                if (rth_entry && (MAX_DAY == 0 || sday_entries < (int)MAX_DAY)) {
                    ...
                    std::printf("[%02d:%02d] *** NOK: PUT *** ~%.2f (blow-off confirmado; "
                                "target %.2f, floor %.2f) t=%.0f\n", H, M, s_entry, s_target, s_floor, b.t);
                    std::fflush(stdout);
                    if (audio_gate(true)) { play("sounds/dram_sell.wav", "Basso"); speak("sell Nokia now"); }
                    { char m[200]; std::snprintf(m, sizeof(m),
                        "VENDER NOK @ %.2f | target %.2f | floor %.2f | senal de BAJADA (blow-off)",
