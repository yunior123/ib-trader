    ...
    while (std::fgets(line, sizeof(line), in)) {
        ...
        if (pending_buy && !in_pos) {
            ...
            if (rth_entry && (MAX_DAY == 0 || day_entries < (int)MAX_DAY)) {
                ...
                { char m[200]; std::snprintf(m, sizeof(m),
                    "COMPRAR NVDA @ %.2f (shares o CALL) | target %.2f | floor %.2f | capitulacion confirmada",
        ...
        if (SHORTS > 0) {
            ...
            if (pending_short && !in_short && !in_pos) {
                ...
                if (rth_entry && (MAX_DAY == 0 || sday_entries < (int)MAX_DAY)) {
                    ...
                    std::fflush(stdout);
                    if (audio_gate(true)) { play("sounds/dram_sell.wav", "Basso"); speak("buy NVIDIA put now"); }
                    { char m[200]; std::snprintf(m, sizeof(m),
                        "COMPRAR PUT NVDA @ %.2f | target %.2f | floor %.2f | senal de BAJADA (blow-off)",
                        s_entry, s_target, s_floor);
                      notify("NVDA: BUY PUT", m, true); }
                }
            }
