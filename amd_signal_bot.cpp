// amd_signal_bot.cpp — AMD buy/sell signal engine, pure C++ (clon del motor NOK, 2026-07-10; gold/nas100 = proxies GLD/QQQ)
// ============================================================
...
int main(int argc, char** argv) {
    ...
    while (std::fgets(line, sizeof(line), in)) {
        ...
        if (nbars > 1 && prev_close > 0) { /* prev_close ya actualizado: usar retorno del bar */ }
        {
            ...
            if (last_c_for_ret > 0) {
                ...
                if (alert_hours && b.t - last_cusum > 3600) {
                    if (cusum_up > hthr) {
                        std::printf("[%02d:%02d] CUSUM: AMD SUBIENDO fuerte (+%.2f%% acumulado) px %.2f\n",
                                    H, M, cusum_up * 100, b.c);
                        std::fflush(stdout);
                        { char m[160]; std::snprintf(m, sizeof(m), "CUSUM: subiendo fuerte %+.2f%% px %.2f", cusum_up*100, b.c); notify("AMD alza", m, false); }
                        cusum_up = 0; cusum_dn = 0; last_cusum = b.t;
                    } else if (cusum_dn < -hthr) {
                        std::printf("[%02d:%02d] CUSUM: AMD CAYENDO fuerte (%.2f%% acumulado) px %.2f\n",
                                    H, M, cusum_dn * 100, b.c);
                        std::fflush(stdout);
                        { char m[160]; std::snprintf(m, sizeof(m), "CUSUM: cayendo fuerte %.2f%% px %.2f", cusum_dn*100, b.c); notify("AMD caida", m, false); }
                        cusum_up = 0; cusum_dn = 0; last_cusum = b.t;
                    }
                }
            }
        ...
        if (in_pos && b.t >= pos_epoch) {
            ...
            if (sold) {
                std::printf("[%02d:%02d] *** AMD: VENDER *** ~%.2f (%s, entrada %.2f)\n",
        ...
        if (pending_buy && !in_pos) {
            ...
            if (rth_entry && (MAX_DAY == 0 || day_entries < (int)MAX_DAY)) {
                ...
                { char m[200]; std::snprintf(m, sizeof(m),
                    "COMPRAR AMD @ %.2f | target %.2f (+4%%) | floor %.2f | capitulacion confirmada",
