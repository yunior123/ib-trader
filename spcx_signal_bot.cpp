...
// SPCX_MODE=trend (estudio 30d 2026-07-10): el dip-engine confirmo 0% en SPCX
// (waterfalls sin reclaim; follow-through promedio NEGATIVO con dips
// relajados) y sus rangos de dia ~9% premian montarse al movimiento, no
// esperar RSI25. Money-path alternativo: entra en flip alcista del
// Supertrend 5m o ruptura del maximo del dia con CUSUM construyendo; sale en
// flip bajista / trail / EOD forzado. OFF por defecto hasta validarlo.
...
int main(int argc, char** argv) {
    ...
    if (!use_stdin) {
        ...
        // (fix 2026-07-10: Polygon ws nunca conecto — plan sin acceso ws)
    ...
    while (std::fgets(line, sizeof(line), in)) {
        ...
        // ---- TREND MODE: señal de entrada (ver nota en la cabecera) ----
        ...
        if (TREND_MODE && !in_pos && !pending_buy && nbars > 30) {
            ...
            if (trend_rth && (flip_up || don_break) && cusum_up >= 0.01) {
                pending_buy = true;
                std::printf("[%02d:%02d] TREND-ENTRY armado: %s px %.2f (CUSUM +%.1f%%)\n",
        ...
        if (in_pos && b.t >= pos_epoch) {
            ...
            if (sold) {
                ...
                std::fflush(stdout);
                if (audio_gate(true)) { play("sounds/dram_sell.wav", "Hero"); speak("sell SpaceX now"); }
        ...
        if (pending_buy && !in_pos) {
            ...
            if (rth_entry && (MAX_DAY == 0 || day_entries < (int)MAX_DAY)) {
                ...
                std::fflush(stdout);
                if (audio_gate(true)) { play("sounds/dram_buy.wav", "Glass"); speak("buy SpaceX now"); }
                { char m[200]; std::snprintf(m, sizeof(m),
