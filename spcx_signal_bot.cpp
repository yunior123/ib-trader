...
//
// Data: scripts/ws_bar_bridge.py polygon SPCX spcx streams REAL Yahoo 1m completed bars as
...
int main(int argc, char** argv) {
    ...
    FILE* in = stdin;
    if (!use_stdin) {
        in = popen("venv/bin/python scripts/ws_bar_bridge.py polygon SPCX spcx 2>/dev/null", "r");
    ...
    while (std::fgets(line, sizeof(line), in)) {
            ...
            if (last_c_for_ret > 0) {
                ...
                if (alert_hours && b.t - last_cusum > 3600) {
                    if (cusum_up > hthr) {
                        ...
                        std::fflush(stdout);
                        if (audio_gate(false)) { play("sounds/momentum_up.wav", "Ping"); speak("SpaceX rising fast"); }
                        ...
                        std::fflush(stdout);
                        if (audio_gate(false)) { play("sounds/momentum_down.wav", "Basso"); speak("SpaceX falling fast"); }
        ...
        if (bar5 && atr5 > 0 && n5 > 10) {
            ...
            if (st_trend != 0 && nt != st_trend && alert_hours && b.t - last_st > 3600) {
                if (nt > 0) {
                    ...
                    if (audio_gate(false)) { play("sounds/momentum_up.wav", "Ping"); speak("SpaceX trend is now up"); }
                    ...
                    if (audio_gate(false)) { play("sounds/momentum_down.wav", "Basso"); speak("SpaceX trend is now down"); }
        ...
        if ((int)dh20.size() == 390 && alert_hours && b.t - last_don > 3600) {
            ...
            if (b.c > hi) {
                ...
                std::fflush(stdout);
                if (audio_gate(false)) { play("sounds/momentum_up.wav", "Ping"); speak("SpaceX breaking out"); }
                ...
                std::fflush(stdout);
                if (audio_gate(false)) { play("sounds/momentum_down.wav", "Basso"); speak("SpaceX breaking down"); }
