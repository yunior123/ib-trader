...
- **BULL: "we buy and sell higher only, if not we keep the bag"** — vender SOLO ≥ entry+1% (`ETF_BAG_MIN_GAIN`); señal de venta por debajo ⇒ BOLSA + limit **GTC** de recuperacion EN EL BROKER (sobrevive reinicios/muerte del Mac). Sin stop loss en bulls, Yunior aguanta la bolsa con gusto (sus favoritos).
- **BEAR: jamas bolsa** — STOP GTC servidor-side a −5% (`ETF_BEAR_STOP`) SIEMPRE; si el stop no se puede colocar, se cierra la posicion al instante. Sale ademas con la señal de cover del bot.
...
- **Activacion**: existe `data/etf_armed` (creado 2026-07-11) **y** NetLiq ≥ 450 USD (`ETF_MIN_EQUITY`) → opera solo. **KILL SWITCH: `rm data/etf_armed`** (dry-run inmediato). Verificado en vivo: señal inyectada → "SKIP equity 68 USD < 450 — esperar fondeo".
