...
- **Prohibido renombrar credenciales** pensando que "ya no son activas". `.apagado`, `.bak`,
  `*borrados*`, `*snapshot*` — NINGUNO de esos nombres es seguro. Si un fichero tuvo credenciales,
  **se borra del disco**, no se renombra. `rm`, no `mv`.
- **Los hooks de git bloquean commits/pushes con secretos** (`scripts/git-secrets-check.sh`).
  Si falla un commit, NO lo fuerces con `--no-verify`. Arregla el fichero.
- **Si un token se filtró (chat, GitHub, log): se rota INMEDIATAMENTE.**
  El 2026-08-23 costó 35 webhooks filtrados + server spameado con @everyone.
...
0. **LEY DE EJECUCION (base 2026-07-16 + ENMIENDA 2026-07-24).** POR DEFECTO toda la flota — los `*_signal_bot`, scanners, alarmas, narrador, qqq_xray, TODO — es SEÑAL-SOLAMENTE: jamas coloca/modifica/cancela ordenes; solo `BUY`/`SELL` + `prob NN%` + razones; el humano opera. Los ejecutores VIEJOS siguen RETIRADOS en `backup/execution_retired_2026-07-16/` y `backup/executors_retired_2026-07-15/` — NO revivirlos ni importarlos (perdieron dinero: GTC/OCA residuales del broker vendieron INTW/RAM con el executor caido; matar PIDs locales NO basta).
   **ENMIENDA (Yunior 2026-07-24, orden explicita nueva — reversa consciente de la ley 2026-07-16, elegida con paper-first y watch-local):** se AUTORIZA UN UNICO modulo de ejecucion, **`order_engine/`** (C++23, TWS C++ API oficial), a colocar/modificar/cancelar ordenes de OPCIONES (calls+puts, CUALQUIER expiry, no solo 0DTE). Reglas NO negociables del modulo (fuera de estas sigue rigiendo señal-solamente):
   - (a) **DOBLE LLAVE**: flag `--arm-live` + archivo `order_engine/ARM_LIVE` con la fecha de HOY. Sin AMBAS → corre DRY/SIM (imprime la orden que colocaria, no toca cuenta).
   - (b) **PAPER PRIMERO**: puerto **7497** (paper DUR197573) hasta que buy/sell/cancel/modify + el cleanup esten probados; **7496** (live TFSA U26942420) solo tras armar explicito.
   - (c) **DISARM-ON-EXIT OBLIGATORIO**: al salir / crash / `connectionClosed` cancela TODAS sus ordenes propias y congela entradas. JAMAS dejar residual server-side — ESA fue la causa del desastre 2026-07-16.
   - (d) **Entradas = watch LOCAL al PRINT** (2 lecturas, doctrina PRINT-O-NADA): la orden vive local hasta el trigger, no descansa en el servidor. Solo los STOP protectivos van nativos server-side (sobreviven crash = proteccion) y SIEMPRE dentro del set de cancel-all.
   - (e) **Topes**: premium ≤ cap ($200 default), spread ≤ 5%, OI > 500; clientId dedicado (no reusar 48/82/83/84/87/90).
   Ningun otro proceso de la flota ordena. Arquitectura y fases: **`order_engine/docs/ORDER-ENGINE.md`**. Skill de conexion: `.claude/skills/ibkr-tws`.
