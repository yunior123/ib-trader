...
if ! ( : > "$SIGDIR/.perm_check" ) 2>/dev/null; then
  ...
  "$IBT_OSA" -e 'display notification "Bots SIN permiso (TCC). Activa Full Disk Access al runner." with title "🔴 PERMISO DENEGADO" sound name "Sosumi"' 2>/dev/null
else
...
if [[ ! -x "$FLEET_HOURS" ]]; then
  ...
  if [[ ! -f "$ROOT/data/FLEET_HOURS_MISSING" ]]; then
    ...
    "$IBT_OSA" -e 'display notification "Falta ./fleet_hours: la flota NO arranca. Corre scripts/build_fleet_hours.sh" with title "🔴 PORTERO AUSENTE" sound name "Sosumi"' 2>/dev/null
  fi
  fleet_stop_all
  exit 0
fi
...
if [[ -f "$ROOT/data/fleet_sleep" ]]; then
  ...
  if [[ -n "$WAKE" && "$(date +%s)" -ge "$WAKE" ]]; then
    ...
    "$IBT_OSA" -e 'display notification "La flota amaneció sola: bots, sirenas y feeds armados. Buen OPEX." with title "🌅 FLOTA DESPIERTA" sound name "ProChord"' 2>/dev/null
  else
    fleet_stop_all
...
# Con data/notify_off no se relanza ningún relé: el silencio sobrevive a un reinicio de la flota.
if [ ! -f "$ROOT/data/notify_off" ] && ! pgrep -f "scripts/notify_relay.sh" >/dev/null; then
...
if [ ! -f "$ROOT/data/notify_off" ] && ! pgrep -f "voice_queue.sh" >/dev/null; then
