...
# Interruptor duro de notificaciones (data/notify_off): ni voz ni banner en el Mac.
[ -z "${PYTEST_CURRENT_TEST:-}" ] && [ -f "$ROOT/data/notify_off" ] && exit 0
...
if [ "$PRIO" != "DANGER" ] && [ -f "$ROOT/data/voice_budget_enable" ]; then
  ...
  if [ -x "$PY" ]; then
    ...
    if [ "$?" = "42" ]; then
      "$IBT_OSA" -e "display notification \"$(printf '%s' "$MSG" | tr -d '"')\" with title \"🔇 presupuesto de voz\"" >/dev/null 2>&1 &
      exit 0
    fi
  fi
fi
