    ...
    if nc -z -w2 127.0.0.1 7496 2>/dev/null; then
      ...
      if is_zombie; then
        ...
        if [[ $fails -ge 3 && $(( $(date +%s) - last_action )) -ge 900 ]]; then
          ...
          sleep 5
          launch_tws
          "$IBT_OSA" -e 'display notification "TWS ZOMBIE (puerto vivo, 0 bars 13 min) — relanzado, LOGIN + 2FA requerido" with title "🧟 TWS WATCHDOG" sound name "ProAlarm"' 2>/dev/null
      ...
      if [[ $awaiting_login -eq 1 ]]; then
        ...
        if ! tws_alive && [[ $(( $(date +%s) - last_action )) -ge 180 ]]; then
          last_action=$(date +%s)
          launch_tws
          "$IBT_OSA" -e 'display notification "TWS murio esperando login — relanzado OTRA VEZ, LOGIN + 2FA requerido" with title "🚨 TWS WATCHDOG" sound name "ProAlarm"' 2>/dev/null
          ...
          last_action=$(date +%s)
          "$IBT_OSA" -e 'display notification "TWS sigue esperando tu LOGIN + 2FA" with title "🔑 LOGIN TWS PENDIENTE" sound name "ProChord"' 2>/dev/null
        fi
        fails=0
        ...
        sleep 5
        launch_tws
        "$IBT_OSA" -e 'display notification "TWS caido — relanzado, LOGIN + 2FA requerido" with title "🚨 TWS WATCHDOG" sound name "ProAlarm"' 2>/dev/null
