...
while true; do
  ...
  rc=$?
  if [ "$rc" -eq 78 ]; then
    echo "$(date) opt_sentinel RETIRADO (exit 78, datos 2026-07-16 vencidos) — keepalive se apaga" >> opt_sentinel.log
    exit 0
  fi
  echo "$(date) opt_sentinel salio (rc $rc); relanzando" >> opt_sentinel.log
  sleep 20
done
