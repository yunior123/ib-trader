...
# ARRAY, no escalar: zsh no hace word-splitting, asi que "-Wall -Wextra" llegaba a clang++
# como UN solo argumento -> "unknown warning option" y NINGUNA de las dos avisos activa.
# Mismo bug que se documenta abajo para BOTS. Medido el 2026-08-23.
WARN=( -Wall -Wextra )
FAILED=0
WARNED=0
...
for src in scripts/flow_pulse.cpp scripts/qqq_xray.cpp scripts/price_alarm.cpp scripts/korea_watch.cpp scripts/finviz_scout.cpp "${BOTS[@]}"; do
  ...
  # A bin/, que es de donde los lanza la flota. Compilando a la raiz el deploy decia OK y los
  # procesos seguian con el binario viejo (precedente: la mudanza a bin/ dejo la flota muerta).
  mkdir -p bin
  if clang++ $STD -O3 $ARCH "${WARN[@]}" -o "bin/$out" "$src" $LIBS 2>/tmp/cc_err; then
    nw=$(grep -c "warning:" /tmp/cc_err)
    if [ "$nw" -gt 0 ]; then echo "  ⚠️  $out ($nw warnings)"; head -3 /tmp/cc_err; WARNED=$((WARNED+1));
    else echo "  ✅ $out"; fi
    # lse_price_alarm es el MISMO binario que price_alarm con otro nombre (lo lanza
    # lse_price_alarm_keepalive.sh). Sin esta copia se quedaba con la version vieja.
    [ "$out" = "price_alarm" ] && cp -f bin/price_alarm bin/lse_price_alarm
  else
