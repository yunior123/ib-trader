...
static bool read_nbbo(const std::string &sym, long &ts, double &mid) {
    ...
    if (!f) return false;
    // epoch en double: lse_price_alarm_feed.py lo escribe con decimales ("%.6f") y leerlo
    // como long dejaba ".290011" en el flujo -> ese resto se leia como BID (TSLA 170,46 = mitad).
    double ep, bid, ask;
    if (!(f >> ep >> bid >> ask)) return false;
...
static void notify(const std::string &msg) {
    ...
    std::string cmd = "./scripts/osa_gate -e 'delay (random number from 0.0 to 0.45)' "
