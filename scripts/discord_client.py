...
def escritura_bloqueada(method):
    """Con data/notify_off puesto, NADA escribe en Discord: ni mensajes, ni webhooks, ni canales.

    La guarda va AQUI, en el cliente que usan todos los scripts, y no en cada uno: el 2026-08-23
    se apago discord_send y aun asi se recrearon 37 canales y una tanda de webhooks, porque otros
    modulos llaman a request() directamente. Se permite LEER (GET) para poder auditar."""
    if method.upper() == "GET":
        return False
    off = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "data", "notify_off")
...
def request(method, path, payload=None, tok=None, reason=None):
    ...
    if escritura_bloqueada(method):
        raise DiscordError("escritura bloqueada: data/notify_off presente (%s %s)" % (method, path))
    tok = tok or token()
    if not tok:
