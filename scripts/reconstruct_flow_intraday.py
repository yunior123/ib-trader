...
data/backtest/flow_intraday_<sym>.csv (epoch_hora,volC_acum_dia,volP_acum_dia,pc,spot).
...
def recon(sym):
    ...
    for exp in fridays(d0, d1 + 7 * 86400):
        ...
        for otk, typ in contracts:
            d = poly(f"https://api.polygon.io/v2/aggs/ticker/{otk}/range/1/hour/{wk_from}/{exp}?adjusted=true&limit=5000&sort=asc")
            for r in d.get("results", []):
                he = (r["t"] // 1000) // 3600 * 3600
    ...
    print(f"{sym.upper()}: {len(rows)} horas de flujo intradia -> {path}")
