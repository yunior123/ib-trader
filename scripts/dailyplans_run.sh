...
./venv/bin/python scripts/gex_snapshot.py >> dailyplans.log 2>&1
...
[[ $MODE == FULL ]] && ./volume_profile >> dailyplans.log 2>&1
[[ $MODE == FULL ]] && ./venv/bin/python scripts/kde_levels.py >> dailyplans.log 2>&1
...
if [[ $MODE == FULL ]]; then
  ...
  if (( GEX_N == 0 )) || (( GEX_AGE > 129600 )) || (( GEX_N * 2 < FLEET_N )); then
    echo "$(date) ⚠ MAPA GAMMA ausente/rancio/incompleto (${GEX_N}/${FLEET_N} syms, ${GEX_AGE}s) — los planes NO afirman regimen gamma" >> dailyplans.log
    osascript -e 'display notification "Mapa gamma incompleto: planes sin regimen gamma" with title "⚠ ib-trader 4AM"' 2>/dev/null
  else
    echo "$(date) mapa gamma MEDIDO ok: ${GEX_N}/${FLEET_N} syms, ${GEX_AGE}s" >> dailyplans.log
  fi
