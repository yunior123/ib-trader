...
if [[ $MODE == FULL ]]; then
  ...
  if (( GEX_N == 0 )) || (( GEX_AGE > 129600 )) || (( GEX_N * 2 < FLEET_N )); then
    ...
    "$IBT_OSA" -e 'display notification "Mapa gamma incompleto: planes sin regimen gamma" with title "⚠ ib-trader 4AM"' 2>/dev/null
  else
...
"$IBT_OSA" -e 'display notification "📋 Planes flota generados + email + X" with title "ib-trader"'
