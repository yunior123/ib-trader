...
status() {
  ...
  # El centinela de sueño manda sobre TODO: sin gritarlo, el ✗ de cada símbolo miente sobre
  # la causa (2026-08-14: 6 llamadas para descubrir que la flota estaba dormida a propósito).
  if [[ -f "$ROOT/data/fleet_sleep" ]]; then
    local _sl="$(head -1 "$ROOT/data/fleet_sleep" 2>/dev/null)"
    local _wk=$(grep -o 'wake: [0-9]*' "$ROOT/data/fleet_sleep" 2>/dev/null | awk '{print $2}')
    print -P "  %F{yellow}!!%f %BFLOTA DORMIDA A PROPÓSITO%b — data/fleet_sleep: ${_sl:-(sin texto)}"
    if [[ -n "$_wk" ]]; then
      print -P "     despierta sola el $(date -r "$_wk" '+%F %H:%M' 2>/dev/null || print "epoch $_wk")"
    else
      print -P "     %F{red}SIN campo 'wake:' → sueño INDEFINIDO%f: no despierta sola."
      print -P "     Para despertarla: rm data/fleet_sleep && zsh scripts/fleet_up.sh"
    fi
    print -P "     Mientras exista, fleet_keepalive_start.sh para todo y sale (linea 139)."
  fi
  local fleet_live=0
