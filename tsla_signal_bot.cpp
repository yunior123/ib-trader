            ...
            if (rth_entry && (MAX_DAY == 0 || day_entries < (int)MAX_DAY)) {
                if (in_short) {   // reversal: capitulacion confirmada = cubrir corto
                    ...
                    std::printf("[%02d:%02d] *** TSLA: VENDER PUT *** ~%.2f (reversal a largo, entrada %.2f) t=%.0f\n",
                                H, M, px, s_entry, b.t);
                    std::fflush(stdout);
                    if (audio_gate(true)) { play("sounds/dram_buy.wav", "Glass"); speak("buy Tesla call now"); }
                    { char m[200]; std::snprintf(m, sizeof(m),
                        "COMPRAR CALL TSLA @ %.2f | reversal a alza | entrada %.2f | mov %+.1f%%",
                        px, s_entry, (s_entry / px - 1) * 100);
                      notify("TSLA: BUY CALL", m, true); }
                ...
                { char m[200]; std::snprintf(m, sizeof(m),
                    "COMPRAR TSLA @ %.2f (shares o CALL) | target %.2f | floor %.2f | capitulacion confirmada",
        ...
        if (SHORTS > 0) {
            ...
            if (in_short && b.t >= spos_epoch) {
                ...
                if (cov) {
                    ...
                    std::fflush(stdout);
                    if (audio_gate(true)) { play("sounds/dram_buy.wav", "Ping"); speak("buy Tesla call now"); }
                    { char m[200]; std::snprintf(m, sizeof(m),
                        "COMPRAR CALL TSLA @ %.2f | %s | entrada %.2f | mov %+.1f%%",
                        exit_px, why, s_entry, (s_entry / exit_px - 1) * 100);
                      notify(why[0] == 'H' ? "TSLA: PUT-STOP" : "TSLA: BUY CALL", m, true); }
            ...
            if (pending_short && !in_short && !in_pos) {
                ...
                if (rth_entry && (MAX_DAY == 0 || sday_entries < (int)MAX_DAY)) {
                    ...
                    std::fflush(stdout);
                    if (audio_gate(true)) { play("sounds/dram_sell.wav", "Basso"); speak("buy Tesla put now"); }
                    { char m[200]; std::snprintf(m, sizeof(m),
                        "COMPRAR PUT TSLA @ %.2f | target %.2f | floor %.2f | senal de BAJADA (blow-off)",
                        s_entry, s_target, s_floor);
                      notify("TSLA: BUY PUT", m, true); }
                }
            }
