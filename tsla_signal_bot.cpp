    ...
    while (std::fgets(line, sizeof(line), in)) {
        ...
        if (pending_buy && !in_pos) {
            ...
            if (rth_entry && (MAX_DAY == 0 || day_entries < (int)MAX_DAY)) {
                if (in_short) {   // reversal: capitulacion confirmada = cubrir corto
                    double px = b.o;
                    std::printf("[%02d:%02d] *** TSLA: CUBRIR *** ~%.2f (reversal a largo, entrada %.2f)\n",
                                H, M, px, s_entry);
                    std::fflush(stdout);
                    if (audio_gate(true)) { play("sounds/dram_buy.wav", "Glass"); speak("cover Tesla now"); }
                    { char m[200]; std::snprintf(m, sizeof(m),
                        "CUBRIR TSLA @ %.2f | reversal a largo | entrada %.2f | PnL %+.1f%%",
                        px, s_entry, (s_entry / px - 1) * 100);
                      notify("TSLA: COVER NOW", m, true); }
        ...
        if (SHORTS > 0) {
            ...
            if (in_short && b.t >= spos_epoch) {
                ...
                if (cov) {
                    std::printf("[%02d:%02d] *** TSLA: CUBRIR *** ~%.2f (%s, entrada %.2f)\n",
                                H, M, exit_px, why, s_entry);
                    std::fflush(stdout);
                    if (audio_gate(true)) { play("sounds/dram_buy.wav", "Ping"); speak("cover Tesla now"); }
                    { char m[200]; std::snprintf(m, sizeof(m),
                        "CUBRIR TSLA @ %.2f | %s | entrada %.2f | PnL %+.1f%%",
                        exit_px, why, s_entry, (s_entry / exit_px - 1) * 100);
                      notify(why[0] == 'H' ? "TSLA: COVER-STOP" : "TSLA: COVER NOW", m, true); }
            ...
            if (pending_short && !in_short && !in_pos) {
                ...
                if (rth_entry && (MAX_DAY == 0 || sday_entries < (int)MAX_DAY)) {
                    ...
                    std::printf("[%02d:%02d] *** TSLA: SHORT *** ~%.2f (blow-off confirmado; "
                                "target %.2f, floor %.2f)\n", H, M, s_entry, s_target, s_floor);
                    std::fflush(stdout);
                    if (audio_gate(true)) { play("sounds/dram_sell.wav", "Basso"); speak("short Tesla now"); }
                    { char m[200]; std::snprintf(m, sizeof(m),
                        "SHORT TSLA @ %.2f | target %.2f | floor %.2f | blow-off confirmado",
                        s_entry, s_target, s_floor);
                      notify("TSLA: SHORT NOW", m, true); }
                }
            }
