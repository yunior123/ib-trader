// spcx SPCX SpaceX polygon_signal_bot.cpp —  buy/sell signal engine, pure C++
// ============================================================
...
//
// Data: scripts/spcx SPCX SpaceX polygon_bar_bridge.py streams REAL Yahoo 1m completed bars as
//       "EPOCH OPEN HIGH LOW CLOSE VOLUME" lines (stored locally by bridge).
//
// build: clang++ -std=c++17 -O2 -o spcx SPCX SpaceX polygon_signal_bot spcx SPCX SpaceX polygon_signal_bot.cpp
// run:   ./spcx SPCX SpaceX polygon_signal_bot            (spawns bridge)
//        ./spcx SPCX SpaceX polygon_signal_bot --stdin    (replay: feed bar lines for testing)
...
static void notify(const char* title, const char* msg, bool urgent) {
    ...
    FILE* f = std::fopen("spcx SPCX SpaceX polygon_operations.log", "a");
    if (f) {
...
int main(int argc, char** argv) {
    ...
    FILE* in = stdin;
    if (!use_stdin) {
        in = popen("venv/bin/python scripts/ws_bar_bridge.py   spcx SPCX SpaceX polygon 2>/dev/null", "r");
        if (!in) { std::fprintf(stderr, "no bridge\n"); return 1; }
        std::fprintf(stderr, "spcx SPCX SpaceX polygon_signal_bot (C++): bridge  1m real iniciado\n");
    }
    ...
    while (std::fgets(line, sizeof(line), in)) {
        ...
        if (nbars > 1 && prev_close > 0) { /* prev_close ya actualizado: usar retorno del bar */ }
        {
            ...
            if (last_c_for_ret > 0) {
                ...
                if (alert_hours && b.t - last_cusum > 3600) {
                    if (cusum_up > hthr) {
                        std::printf("[%02d:%02d] CUSUM:  SUBIENDO fuerte (+%.2f%% acumulado) px %.2f\n",
                                    H, M, cusum_up * 100, b.c);
                        std::fflush(stdout);
                        play("sounds/momentum_up.wav", "Ping"); speak(" rising fast");
                        { char m[160]; std::snprintf(m, sizeof(m), "CUSUM: subiendo fuerte %+.2f%% px %.2f", cusum_up*100, b.c); notify(" alza", m, false); }
                        cusum_up = 0; cusum_dn = 0; last_cusum = b.t;
                    } else if (cusum_dn < -hthr) {
                        std::printf("[%02d:%02d] CUSUM:  CAYENDO fuerte (%.2f%% acumulado) px %.2f\n",
                                    H, M, cusum_dn * 100, b.c);
                        std::fflush(stdout);
                        play("sounds/momentum_down.wav", "Basso"); speak(" falling fast");
                        { char m[160]; std::snprintf(m, sizeof(m), "CUSUM: cayendo fuerte %.2f%% px %.2f", cusum_dn*100, b.c); notify(" caida", m, false); }
                        cusum_up = 0; cusum_dn = 0; last_cusum = b.t;
                    }
                }
            }
        ...
        if (bar5 && atr5 > 0 && n5 > 10) {
            ...
            if (st_trend != 0 && nt != st_trend && alert_hours && b.t - last_st > 3600) {
                if (nt > 0) {
                    std::printf("[%02d:%02d] SUPERTREND: tendencia  cambio a ALCISTA px %.2f\n", H, M, b.c);
                    play("sounds/momentum_up.wav", "Ping"); speak(" trend is now up");
                    { char m[120]; std::snprintf(m, sizeof(m), "Supertrend: tendencia ALCISTA px %.2f", b.c); notify(" tendencia", m, false); }
                } else {
                    std::printf("[%02d:%02d] SUPERTREND: tendencia  cambio a BAJISTA px %.2f\n", H, M, b.c);
                    play("sounds/momentum_down.wav", "Basso"); speak(" trend is now down");
                    { char m[120]; std::snprintf(m, sizeof(m), "Supertrend: tendencia BAJISTA px %.2f", b.c); notify(" tendencia", m, false); }
                }
                std::fflush(stdout);
                last_st = b.t;
        ...
        if ((int)dh20.size() == 390 && alert_hours && b.t - last_don > 3600) {
            ...
            if (b.c > hi) {
                std::printf("[%02d:%02d] DONCHIAN:  rompe maximo del dia px %.2f > %.2f\n", H, M, b.c, hi);
                std::fflush(stdout);
                play("sounds/momentum_up.wav", "Ping"); speak(" breaking out");
                { char m[120]; std::snprintf(m, sizeof(m), "Donchian: rompe maximo del dia px %.2f", b.c); notify(" breakout", m, false); }
                last_don = b.t;
            } else if (b.c < lo) {
                std::printf("[%02d:%02d] DONCHIAN:  rompe minimo del dia px %.2f < %.2f\n", H, M, b.c, lo);
                std::fflush(stdout);
                play("sounds/momentum_down.wav", "Basso"); speak(" breaking down");
                { char m[120]; std::snprintf(m, sizeof(m), "Donchian: rompe minimo del dia px %.2f", b.c); notify(" breakdown", m, false); }
                last_don = b.t;
            }
        }
        ...
        if (pending_buy && !in_pos) {
            ...
            if (rth_entry) {
                ...
                std::printf("[%02d:%02d] *** : COMPRAR *** ~%.2f (capitulacion confirmada; "
                            "target %.2f, floor %.2f)\n", H, M, entry, target_px, floor_px);
                std::fflush(stdout);
                play("sounds/dram_buy.wav", "Glass");
                speak("buy  now");
                { char m[200]; std::snprintf(m, sizeof(m),
                    "COMPRAR  @ %.2f | target %.2f (+4%%) | floor %.2f | capitulacion confirmada",
                    entry, target_px, floor_px);
                  notify(": BUY NOW", m, true); }
            }
        }
