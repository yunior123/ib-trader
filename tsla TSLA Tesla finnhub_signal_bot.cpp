// tsla TSLA Tesla finnhub_signal_bot.cpp —  buy/sell signal engine, pure C++
// ============================================================
...
//
// Data: scripts/tsla TSLA Tesla finnhub_bar_bridge.py streams REAL Yahoo 1m completed bars as
//       "EPOCH OPEN HIGH LOW CLOSE VOLUME" lines (stored locally by bridge).
//
// build: clang++ -std=c++17 -O2 -o tsla TSLA Tesla finnhub_signal_bot tsla TSLA Tesla finnhub_signal_bot.cpp
// run:   ./tsla TSLA Tesla finnhub_signal_bot            (spawns bridge)
//        ./tsla TSLA Tesla finnhub_signal_bot --stdin    (replay: feed bar lines for testing)
...
static void notify(const char* title, const char* msg, bool urgent) {
    ...
    FILE* f = std::fopen("tsla TSLA Tesla finnhub_operations.log", "a");
    if (f) {
...
int main(int argc, char** argv) {
    ...
    FILE* in = stdin;
    if (!use_stdin) {
        in = popen("venv/bin/python scripts/ws_bar_bridge.py   tsla TSLA Tesla finnhub 2>/dev/null", "r");
        if (!in) { std::fprintf(stderr, "no bridge\n"); return 1; }
        std::fprintf(stderr, "tsla TSLA Tesla finnhub_signal_bot (C++): bridge  1m real iniciado\n");
    }
