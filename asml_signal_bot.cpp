        ...
        if (in_pos && b.t >= pos_epoch) {
            ...
            if (sold) {
                ...
                { char m[200]; std::snprintf(m, sizeof(m),
                  ...
                  notify(why[0] == 'H' ? "ASML: SELL-STOP" : "ASML: SELL NOW", m, true); }
            ...
            if (rth_entry && (MAX_DAY == 0 || day_entries < (int)MAX_DAY)) {
                if (in_short) {   // reversal: capitulacion confirmada = cubrir corto
                    ...
                    std::printf("[%02d:%02d] *** ASML: VENDER PUT *** ~%.2f (reversal a largo, entrada %.2f) t=%.0f\n",
                                H, M, px, s_entry, b.t);
                    std::fflush(stdout);
                    if (audio_gate(true)) { play("sounds/dram_buy.wav", "Glass"); speak("buy A S M L call now"); }
                    { char m[200]; std::snprintf(m, sizeof(m),
                        "COMPRAR CALL ASML @ %.2f | reversal a alza | entrada %.2f | mov %+.1f%%",
                        px, s_entry, (s_entry / px - 1) * 100);
                      notify("ASML: BUY CALL", m, true); }
                ...
                { char m[200]; std::snprintf(m, sizeof(m),
                    "COMPRAR ASML @ %.2f (shares o CALL) | target %.2f | floor %.2f | capitulacion confirmada",
                    entry, target_px, floor_px);
                  notify("ASML: BUY NOW", m, true); }
            }
        }
        ...
        if (SHORTS > 0) {
            ...
            if (in_short && b.t >= spos_epoch) {
                ...
                if (cov) {
                    ...
                    std::fflush(stdout);
                    if (audio_gate(true)) { play("sounds/dram_buy.wav", "Ping"); speak("buy A S M L call now"); }
                    { char m[200]; std::snprintf(m, sizeof(m),
                        "COMPRAR CALL ASML @ %.2f | %s | entrada %.2f | mov %+.1f%%",
                        exit_px, why, s_entry, (s_entry / exit_px - 1) * 100);
                      notify(why[0] == 'H' ? "ASML: PUT-STOP" : "ASML: BUY CALL", m, true); }
            ...
            if (pending_short && !in_short && !in_pos) {
                ...
                if (rth_entry && (MAX_DAY == 0 || sday_entries < (int)MAX_DAY)) {
                    ...
                    std::fflush(stdout);
                    if (audio_gate(true)) { play("sounds/dram_sell.wav", "Basso"); speak("buy A S M L put now"); }
                    { char m[200]; std::snprintf(m, sizeof(m),
                        "COMPRAR PUT ASML @ %.2f | target %.2f | floor %.2f | senal de BAJADA (blow-off)",
                        s_entry, s_target, s_floor);
                      notify("ASML: BUY PUT", m, true); }
                }
            }
