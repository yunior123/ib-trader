...
# (screener/watchdog.py) owns the SELL regardless of Claude — so a stalled
...
#
# Usage:  screener/claude_trader_loop.sh
...
if [[ -z "$CLAUDE_BIN" ]]; then
  echo "[trader_loop] FATAL: claude binary not found (PATH=$PATH)" >> "$ROOT/screener/trader_loop.err"
  exit 1
fi
...
SETTINGS="$ROOT/screener/claude_settings/settings.json"
PROMPT_FILE="$ROOT/screener/decision_prompt.md"
...
ALIVE="$ROOT/data/screener/claude_alive"   # dead-man ping the watchdog monitors
...
while true; do
  ...
  POS_SYM="$("$PY" "$ROOT/screener/state.py" pos 2>/dev/null)"
  if in_window || [[ -n "$POS_SYM" ]]; then
    ...
    timeout 120 "$CLAUDE_BIN" -p "$PROMPT" \
      ...
      --add-dir "$ROOT" \
      2>>"$ROOT/screener/trader_loop.err" \
      | tee -a "$ROOT/screener/trader_loop.log"
    rc=${pipestatus[1]}
    ...
    echo "[trader_loop] cycle done $(date +%T) rc=$rc" >> "$ROOT/screener/trader_loop.log"
    sleep "$INTERVAL"
  else
