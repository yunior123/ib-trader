...
- [ ] "create studio plan for learning all of quant trading, gamma, charm, delta imbalance... gather youtube videos + websites, open in safari" (2026-08-22) — plan en `docs/ESTUDIO-QUANT.md` (10 modulos, enlaces verificados vivos), abridor `scripts/estudio.sh <0..9>`; M0 abierto en Safari. Estado: **en curso** (M0..M9 por completar, cada modulo cierra con ejercicio medido)

- [x] "mount a website in local host with all the info module by module. like an academy... add exercises in a creative and smart way. surprise me... execute with at least 2 agents... deploy free + free domain... add to my x.com bio" + "make sure users register progress, have their accounts, etc. like a pro website academy. logic is priority" (2026-08-22) — GAMMA DESK, repo `~/quant-academy` (privado en GitHub: quiz con respuestas). DESPLEGADO: https://gammadesk.quant-academy.workers.dev (Cloudflare Workers + D1, gratis). 10 modulos + 10 examenes (79 preguntas, corregidos en servidor) + cuentas con progreso/notas/certificado. Pruebas: 36/36 API y 20/20 navegador, tambien contra produccion. Bio de X ya apunta ahi. Estado: **HECHO** — 7/7 laboratorios, 36/36 API y 28/28 navegador en producción

- [ ] "1.send agent to work on ib trader deployement as well. 2.academy in english too, use some library to have diff languages, search best modern ways to do it lazy, less work, more results. 3. update x.com links when done. 4. make academy simpler, less bla bla and more into learning and dynamic exercises, nice path to track progress, u can use gemini to verify structure/content, i follow masters like spotgamma, test, fix all, take own best decisions. 5. save knowledge and create new skills" (2026-08-22) — Estado: **en curso**
- [ ] **DECISIÓN**: 39 de 40 jobs `com.ibtrader.*` están deshabilitados en el override de launchd (persiste al reiniciar) y `data/fleet_sleep` lleva activo desde el 2026-08-14 sin `wake:`. Si es intencionado, nada que hacer; si no, la flota NO arrancará sola el domingo a las 20:00. Comando de reactivación en `docs/DEPLOY-STATUS-2026-08-23.md` (2026-08-23, pendiente de Yunior)

## 2026-08-23 — bloqueadores atacados con las fuentes que SOBREVIVIERON

- [x] **Cadena de opciones muerta** (Polygon `/v3/snapshot/options` da 403 en TODOS los símbolos e
      IBKR apagado): `data/opt_chain_<sym>.txt` llevaba sin refrescarse desde el 14-ago y con él
      estaban ciegos `gex_core`, `chart_levels` y `trace_cube`. **RESUELTO** con
      `scripts/cboe_chain_snap.py` (CBOE, gratis, sin clave, con gamma/delta/IV/OI reales) +
      job `com.ibtrader.cboechains` cada 5 min en ventana. Verificado: `gex_core` lee la foto con
      `chain_src: cboe`, `greeks_ok_pct 0,92`, **508 strikes** (antes 56, porque sin `band` en la
      cabecera aplicaba su default de 0,035 y el libro estrecho cambia el GEX de −82M a −111M).
- [x] **`com.ibtrader.tracecube` en fallo desde el 10-ago** («cero fotos utilizables»).
      **RESUELTO y reactivado**: con las fotos de CBOE genera el cubo (178 strikes, GEX medido).
- [x] **`com.ibtrader.fleetnewsrss` en fallo**: era consecuencia de retirar los webhooks de Discord.
      Un canal apagado a propósito no es una avería → con `data/notify_off` puesto se omite la
      publicación y se sale 0; cualquier OTRO fallo de publicación sigue siendo fallo duro.
- [ ] **Flujo de ballenas sin lado agresor — NO se puede arreglar con lo que hay.** UW da 401 y el
      `/options/flow` de LSE trae prima, volumen y las 6 griegas pero **ni `side` ni bid/ask**
      (medido en vivo hoy, y ya estaba escrito en `lse_client.py:25`). Sirve para el archivo y para
      el mapa; **no** para la táctica espada-ballena ni para el gate de convicción, que necesitan
      saber si la prima fue COMPRADA o VENDIDA. Sin agresor, «37k calls» no dice si es techo o
      suelo. Se archiva en Cloudflare (D1, tabla `flujo`) declarando la carencia; no se fabrica.
- [ ] **NBBO**: Polygon `/v2/last/nbbo` da 403 y la capa de proveedores se queda sin quote. Sin gate
