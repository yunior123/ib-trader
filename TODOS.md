...
- [ ] "disable not main symbols like leveraged (DRAM/SPCX/SKHY/EWY) for the moment; commit+push tras review con 5+ agentes; asegurar todo GENÉRICO/dinámico, fácil cambiar data providers; releer market folder por si me perdí features; review review el commit push" (2026-08-01):
      ...
      FOLLOW-UPS del oro NO adoptados (features valiosas, requieren validación antes de señal): (1) motor Bento/Trinity/Router de technical.py (state machine reversión+continuación, solo copiamos los Pine); (2) arnés event-study cross_asset_event_study.py (first-touch/MFE>MAE/Wilson resuelto — encaja con measured-probability); (3) live_htf sin repaint para HTF en los bots C++; (4) shock.py calibrador diario cross-symbol. Arquitecto difirió: __capabilities__ upfront, ProviderError estructurado, observabilidad, shutdown con timeout.
