...
def test_actualizacion_nflx_hace_push_sin_promover_voz_info(tmp_path, monkeypatch):
    ...
    namespace = {
        ...
        # say() llama al portero de banners por su global _OSA (scripts/osa_gate, que respeta
        # data/notify_off). Aqui se ejecuta la funcion suelta, asi que hay que darsela.
        "_OSA": os.path.join(REPO, "scripts", "osa_gate"),
    }
    ...
    # El banner ya no llama a osascript directo: pasa por scripts/osa_gate, que se calla
    # cuando existe data/notify_off. Lo que se comprueba sigue siendo lo mismo: que say()
    # dispara el banner y no la voz.
    assert any(call[0].endswith("osa_gate") for call in popen_calls)
