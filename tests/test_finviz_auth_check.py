...
def test_healthcheck_token_caducado_es_CRIT(monkeypatch):
    """Con la fuente EN ACTIVO, un token caducado sigue siendo critico."""
    monkeypatch.setattr(fh, "fuente_de_baja", lambda nombre: False)
...
def test_healthcheck_token_caducado_de_fuente_dada_de_baja_es_WARN(monkeypatch):
    """Dar de baja una suscripcion es una DECISION, no una averia: baja a aviso, pero se sigue
    diciendo (Yunior 2026-08-23: "only free api, paid api gone all for now")."""
    monkeypatch.setattr(fh, "fuente_de_baja", lambda nombre: nombre == "finviz")
    nivel, msg = fh.finviz_token_status(rec=_rec(sano=False, motivo="HTTP 401"), now=AHORA)
    assert nivel == "warn"
    assert "DADO DE BAJA" in msg
