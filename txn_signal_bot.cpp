        ...
        if (in_pos && b.t >= pos_epoch) {
            ...
            if (sold) {
                ...
                { char m[200]; std::snprintf(m, sizeof(m),
                  ...
                  notify(why[0] == 'H' ? "TXN: SELL-STOP" : "TXN: SELL NOW", m, true); }
            ...
            if (rth_entry && (MAX_DAY == 0 || day_entries < (int)MAX_DAY)) {
                if (in_short) {   // reversal: capitulacion confirmada = cubrir corto
                    ...
                    std::printf("[%02d:%02d] *** TXN: VENDER PUT *** ~%.2f (reversal a largo, entrada %.2f) t=%.0f\n",
                                H, M, px, s_entry, b.t);
                    std::fflush(stdout);
                    if (audio_gate(true)) { play("sounds/dram_buy.wav", "Glass"); speak("buy Texas Instruments call now"); }
                    { char m[200]; std::snprintf(m, sizeof(m),
                        "COMPRAR CALL TXN @ %.2f | reversal a alza | entrada %.2f | mov %+.1f%%",
                        px, s_entry, (s_entry / px - 1) * 100);
                      notify("TXN: BUY CALL", m, true); }
                ...
                { char m[200]; std::snprintf(m, sizeof(m),
                    "COMPRAR TXN @ %.2f (shares o CALL) | target %.2f | floor %.2f | capitulacion confirmada",
                    entry, target_px, floor_px);
                  notify("TXN: BUY NOW", m, true); }
            }
        }
        ...
        if (SHORTS > 0) {
            ...
            if (in_short && b.t >= spos_epoch) {
                ...
                if (cov) {
                    ...
                    std::fflush(stdout);
                    if (audio_gate(true)) { play("sounds/dram_buy.wav", "Ping"); speak("buy Texas Instruments call now"); }
                    { char m[200]; std::snprintf(m, sizeof(m),
                        "COMPRAR CALL TXN @ %.2f | %s | entrada %.2f | mov %+.1f%%",
                        exit_px, why, s_entry, (s_entry / exit_px - 1) * 100);
                      notify(why[0] == 'H' ? "TXN: PUT-STOP" : "TXN: BUY CALL", m, true); }
            ...
            if (pending_short && !in_short && !in_pos) {
                ...
                if (rth_entry && (MAX_DAY == 0 || sday_entries < (int)MAX_DAY)) {
                    ...
                    std::fflush(stdout);
                    if (audio_gate(true)) { play("sounds/dram_sell.wav", "Basso"); speak("buy Texas Instruments put now"); }
                    { char m[200]; std::snprintf(m, sizeof(m),
                        "COMPRAR PUT TXN @ %.2f | target %.2f | floor %.2f | senal de BAJADA (blow-off)",
                        s_entry, s_target, s_floor);
                      notify("TXN: BUY PUT", m, true); }
                }
            }
