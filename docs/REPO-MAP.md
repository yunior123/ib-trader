...
- **charts/live.html** — cockpit vivo del chart: candelas+ribbon+GEX+zonas+ficha 0DTE sobre lightweight-charts v5, alimentado por WebSocket. Clases JS: `BrickView`/`WallBubbles` (ladrillos animados de call wall, put wall, magnet y gamma flip), `GexView`/`GexProfile` (perfil GEX overlay). El estado de golpes/refuerzos/rupturas llega desde `scripts/wall_integrity.py`; London conserva la etiqueta Γ×volumen y bloquea flip sin OI.
...
- **scripts/x_post_common.py** — post/ledger compartido para todos los posters de X (OAuth1, cap duro 30/día $4/mes). Funcs: `load_budget()`, `posts_today_all()`, `make_auth()`, `upload_media()`, `gex_line()`, `post_text()` +4 más.
...
- `test_chart_realtime_fallback.py` (5) — fallback del chart acepta solo prints WS frescos, no retrocede velas ni fabrica volumen, y separa el ritmo tick/barra.
