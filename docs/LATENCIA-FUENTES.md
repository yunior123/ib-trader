...
| **CBOE CDN** | 🔴 **~15 min** (remedido 2026-08-21 **16:04 ET, justo TRAS el cierre — no en sesión**) | El endpoint se llama literalmente `delayed_quotes`. ~~Medido a la vez: `QQQ` 1,8 h · `_SPX` 4,2 h · `SPY` y `SMH` **21,5 h**~~ — **esa toma fue fuera de sesión y no medía nada** (es la trampa que este mismo documento advierte). Remedido el **2026-08-21 16:04 ET** (mercado ya cerrado: es una cota, no un número certificado en RTH — pendiente repetirlo el lunes dentro de sesión), los seis del cockpit a la vez: QQQ **16,1** · NVDA **15,9** · SMH **16,0** · SPY **15,9** · TSLA **15,9** · SPCX **16,0** min. Uniforme, y **la misma latencia que Polygon Starter** | **ESTRUCTURA**: cadena completa con OI+IV+griegas, SPX/XSP/NDX, bid/ask de respaldo. Desde 2026-08-21 es el **primer fallback de OI del cockpit** (`scripts/free_oi.py`): se toma **sólo el `open_interest`** — cifra de apertura que un retraso no cambia — y su IV/griegas delayed se descartan para que London siga aportando spot e IV |
