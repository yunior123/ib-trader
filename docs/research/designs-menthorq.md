...
**What it computes**
1. `DEX_k = Δ_k · OI_k · 100 · S` (dealer sign), `net_dex = Σ DEX_k`. Track intraday `d(net_dex)/dt`. **Encode the sign trap explicitly.** Two fields, never one: `dex_sentiment` and `dex_flow_impact`. ⚠️ **CORREGIDO 2026-07-26**: la frase original de esta ficha decia "positive DEX = bullish customer **but** implies MMs sell underlying", y eso es **fisicamente imposible en la misma convencion**. Con delta CRUDO (lado cliente): DEX>0 = cliente cargado de calls = alcista; el MM VENDIO esas calls, queda CORTO delta y **COMPRA** subyacente para neutralizar. La frase solo seria cierta midiendo el DEX con signo del DEALER, pero entonces "bullish customer" no encaja (dealer largo delta = cliente corto). La implementacion en `gex_core.build_dex` usa la convencion **OI-larga** (la de UW) y la declara en el propio dato con `dex_convention`. Verificado contra el mapa vivo: QQQ net_dex<0 -> `bajista` + `mm_vende`.
