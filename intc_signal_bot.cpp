            ...
            if (rth_entry && (MAX_DAY == 0 || day_entries < (int)MAX_DAY)) {
                if (in_short) {   // reversal: capitulacion confirmada = cubrir corto
                    ...
                    std::printf("[%02d:%02d] *** INTC: VENDER PUT *** ~%.2f (reversal a largo, entrada %.2f) t=%.0f\n",
                                H, M, px, s_entry, b.t);
                    std::fflush(stdout);
                    if (audio_gate(true)) { play("sounds/dram_buy.wav", "Glass"); speak("sell Intel put now"); }
                    { char m[200]; std::snprintf(m, sizeof(m),
                        "VENDER PUT INTC @ %.2f | reversal a alza | entrada %.2f | mov %+.1f%%",
                        px, s_entry, (s_entry / px - 1) * 100);
                      notify("INTC: SELL PUT", m, true); }
        ...
        if (SHORTS > 0) {
            ...
            if (in_short && b.t >= spos_epoch) {
                ...
                if (cov) {
                    ...
                    std::fflush(stdout);
                    if (audio_gate(true)) { play("sounds/dram_buy.wav", "Ping"); speak("sell Intel put now"); }
                    { char m[200]; std::snprintf(m, sizeof(m),
                        "VENDER PUT INTC @ %.2f | %s | entrada %.2f | mov %+.1f%%",
                        exit_px, why, s_entry, (s_entry / exit_px - 1) * 100);
                      notify(why[0] == 'H' ? "INTC: PUT-STOP" : "INTC: SELL PUT", m, true); }
